...
// shared/analysis-temporal-severity.ts
...
var MIN_SAMPLES = 10;
var VALID_BASELINE_TYPES = openapi_filter_param_contracts_default.infrastructureTemporalBaselineTypes;
...
var TEMPORAL_ANOMALIES_TTL = 3600;
...
var CACHE_TTL = 1800;
var NGA_CACHE_KEY = "cable-health-nga-warnings-v1";
var NGA_CACHE_TTL = 86400;
...
async function getCableHealth(_ctx, _req) {
  try {
    const result = await cachedFetchJson(CACHE_KEY, CACHE_TTL, async () => {
      const ngaData = await cachedFetchJson(NGA_CACHE_KEY, NGA_CACHE_TTL, fetchNgaWarnings);
...
init_redis();
var SEED_CACHE_KEY = "cf:radar:ddos:v1";
async function listInternetDdosAttacks(_ctx, _req) {
  try {
    const data = await getCachedJson(SEED_CACHE_KEY, true);
    return {
...
init_redis();
var SEED_CACHE_KEY2 = "infra:outages:v1";
...
async function listInternetOutages(_ctx, req) {
  try {
    const seedData = await getCachedJson(SEED_CACHE_KEY2, true);
...
init_redis();
var SEED_CACHE_KEY3 = "cf:radar:traffic-anomalies:v1";
async function listInternetTrafficAnomalies(_ctx, req) {
  try {
    const data = await getCachedJson(SEED_CACHE_KEY3, true);
  ...
  try {
    ...
    if (service.customParser === "stripe") {
      ...
      if (data2.largestatus === "up") {
        return withStatus("SERVICE_OPERATIONAL_STATUS_OPERATIONAL", data2.message || "All systems operational", latencyMs);
      }
      if (data2.largestatus === "degraded") {
        return withStatus("SERVICE_OPERATIONAL_STATUS_DEGRADED", data2.message || "Degraded performance", latencyMs);
      }
      if (data2.largestatus === "down") {
        return withStatus("SERVICE_OPERATIONAL_STATUS_MAJOR_OUTAGE", data2.message || "Service disruption", latencyMs);
      }
...
async function listServiceStatuses(_ctx, req) {
  try {
    const { data: results, source, leader } = await cachedFetchJsonWithMeta(INFRA_CACHE_KEY, INFRA_CACHE_TTL, async () => {
...
// src/services/wildfires/payload.ts
function resolveFireDetectionTotalCount(response) {
  const reportedTotal = Number(response.pagination?.totalCount);
  return Number.isFinite(reportedTotal) && reportedTotal >= response.fireDetections.length ? reportedTotal : response.fireDetections.length;
}
...
function countSnapshotCoverage(snapshot) {
...
async function writeTemporalAnomaliesSeedMeta(snapshot) {
  return setCachedJson("seed-meta:temporal:anomalies", {
    fetchedAt: Date.now(),
    recordCount: countSnapshotCoverage(snapshot)
  }, 604800).catch(() => false);
}
async function refreshTemporalAnomaliesCacheHit(snapshot) {
...
var CHROME_UA2 = "WorldMonitor/2.0 (https://worldmonitor.app)";
...
var reverseGeocode = async (_ctx, req) => {
  const { lat, lon } = req;
  ...
  if (cached && typeof cached === "object") {
    const normalized = normalizeCacheEntry(cached);
...
var getBootstrapData = async (_ctx, req) => {
