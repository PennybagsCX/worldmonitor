...
function createHealthServiceRoutes(handler, options) {
  return [
    {
      method: "GET",
...
var HEALTH_AIR_QUALITY_KEY = "health:air-quality:v1";
...
init_redis();
var listAirQualityAlerts = async (_ctx, _req) => {
  const payload = await getCachedJson(HEALTH_AIR_QUALITY_KEY, true);
  const sourceStations = payload?.stations ?? payload?.alerts;
  const alerts = normalizeAirQualityStations(sourceStations);
  return {
    alerts,
...
init_redis();
var REDIS_KEY = "health:disease-outbreaks:v1";
var FALLBACK_METHODOLOGY_VERSION = "v1";
var listDiseaseOutbreaks = async (_ctx, _req) => {
