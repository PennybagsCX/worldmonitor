...
var require_stablecoins = __commonJS({
  "shared/stablecoins.json"(exports2, module) {
    module.exports = {
      ids: ["tether", "usd-coin", "dai", "first-digital-usd", "ethena-usde"],
      pegThresholds: {
        onPegMaxDeviation: 5e-3,
...
var require_stablecoin_classifier = __commonJS({
  "shared/stablecoin-classifier.cjs"(exports2, module) {
    "use strict";
    var { pegThresholds: DEFAULT_PEG_THRESHOLDS } = require_stablecoins();
    function toFiniteNumber(value) {
      const n = typeof value === "number" ? value : Number(value);
      return Number.isFinite(n) ? n : 0;
    }
    function classifyStablecoin2(row, thresholds = DEFAULT_PEG_THRESHOLDS) {
      ...
      return {
        ...
        pegStatus: deviation <= thresholds.onPegMaxDeviation ? "ON PEG" : deviation <= thresholds.slightDepegMaxDeviation ? "SLIGHT DEPEG" : "DEPEGGED",
    ...
    module.exports = { classifyStablecoin: classifyStablecoin2 };
  }
});
...
var import_stablecoins = __toESM(require_stablecoins(), 1);
...
async function fetchCoinGeckoMarkets(ids, opts = {}) {
...
var COINPAPRIKA_ID_MAP = {
  ...
  ...import_stablecoins.default.coinpaprika
};
var COINPAPRIKA_FETCH_CONCURRENCY = 4;
async function fetchCoinPaprikaTickersById(paprikaIds) {
  const ids = [...new Set(paprikaIds.filter((id) => Boolean(id)))];
  ...
  for (const [index, result] of results.entries()) {
    if (result.status === "fulfilled") {
      ...
      console.warn(`[CoinPaprika] Skipping ${ids[index] ?? "unknown"}:`, result.reason.message || result.reason);
    }
  }
...
async function fetchCoinPaprikaMarkets(geckoIds) {
  const paprikaIds = geckoIds.map((id) => COINPAPRIKA_ID_MAP[id]).filter((id) => Boolean(id));
...
async function fetchCryptoMarketsWithSource(ids, opts = {}) {
  try {
...
// server/worldmonitor/market/v1/_quote-provider.ts
var QUOTE_PROVIDER_TIMEOUT_MS = 3e3;
function providerSignal(parent) {
  const controller = new AbortController();
  const timer = setTimeout(
...
var FINNHUB_UNSUPPORTED = new Set(stocks_default.yahooOnly);
function isYahooNotationSymbol(symbol) {
  return symbol.startsWith("^") || symbol.endsWith("=F") || symbol.endsWith("=X");
}
function isEquityServiceableSymbol(symbol) {
  return !FINNHUB_UNSUPPORTED.has(symbol) && !isYahooNotationSymbol(symbol);
}
var FinnhubQuoteProvider = class {
  constructor(apiKey) {
    this.apiKey = apiKey;
  }
  apiKey;
  ...
  async fetchQuote(symbol, requestSignal) {
    ...
    try {
      ...
      if (!Number.isFinite(data.c) || data.c <= 0) return { status: "not_found" };
      return {
        status: "ok",
        quote: {
          price: data.c,
...
var AlphaVantageQuoteProvider = class {
  constructor(apiKey) {
    this.apiKey = apiKey;
  }
  apiKey;
  ...
  async fetchQuote(symbol, requestSignal) {
    const attempt = providerSignal(requestSignal);
    try {
      const url = `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${encodeURIComponent(symbol)}&apikey=${encodeURIComponent(this.apiKey)}`;
      const resp = await fetch(url, {
        headers: { Accept: "application/json", "User-Agent": CHROME_UA },
        signal: attempt.signal
      });
...
var CascadeQuoteProvider = class {
...
function resolveMarketQuoteProvider(opts = {}) {
  const finnhubKey = opts.finnhubKey === void 0 ? process.env.FINNHUB_API_KEY : opts.finnhubKey;
  const avKey = opts.alphaVantageKey === void 0 ? process.env.ALPHA_VANTAGE_API_KEY : opts.alphaVantageKey;
  const providers = [];
...
function providerNotConfiguredReason() {
  return "FINNHUB_API_KEY and ALPHA_VANTAGE_API_KEY not configured";
}

// server/worldmonitor/market/v1/list-market-quotes.ts
init_redis();
var BOOTSTRAP_KEY = "market:stocks-bootstrap:v1";
var QUOTE_CACHE_KEY_PREFIX = "market:custom-quote:v1";
var QUOTE_CACHE_TTL_SECONDS = 300;
var QUOTE_NEGATIVE_TTL_SECONDS = 900;
var MARKET_QUOTES_REQUEST_LIMIT = 120;
var MARKET_QUOTES_UPSTREAM_LIMIT = 10;
var UPSTREAM_CONCURRENCY = 4;
var UPSTREAM_DEADLINE_MS = 6e3;
var upstreamDeadlineMs = UPSTREAM_DEADLINE_MS;
var QUOTE_PROVIDER_CALL_BUDGET_MS = 5e3;
var CALL_TIMEOUT_DEADLINE_GRACE_MS = 50;
var REASON = {
...
function normalizeRequestedSymbols(raw, limit = MARKET_QUOTES_REQUEST_LIMIT) {
...
function filterMarketQuotes(bootstrap, symbols) {
  if (symbols.length === 0) return bootstrap;
  const bySymbol = new Map(bootstrap.quotes.map((quote) => [quote.symbol, quote]));
  return {
    ...bootstrap,
    quotes: symbols.map((symbol) => bySymbol.get(symbol)).filter((quote) => quote != null)
  };
}
...
function gapFetchCallTimeoutMs(remainingMs) {
  const remaining = Math.max(0, remainingMs);
  return remaining <= QUOTE_PROVIDER_CALL_BUDGET_MS ? remaining + CALL_TIMEOUT_DEADLINE_GRACE_MS : QUOTE_PROVIDER_CALL_BUDGET_MS;
}
function classifyGapFetchFailure(input) {
...
async function resolveMissingSymbols(missing) {
  ...
  const liveOutcomes = /* @__PURE__ */ new Map();
  const cutoffs = [];
  await forEachWithConcurrency(attempted, UPSTREAM_CONCURRENCY, async (symbol) => {
    ...
    try {
      const cached = await waitForAbort(cachedFetchJson(
...
function seedUnavailableResponse(symbols) {
  return {
    quotes: [],
...
async function listMarketQuotes(_ctx, req) {
  ...
  const bootstrap = seed.status === "hit" ? seed.value : null;
  ...
  const overflow = dropped.map((symbol) => ({
    symbol,
...
init_redis();
var SEED_CACHE_KEY = "market:crypto:v1";
var GAP_CACHE_TTL = 600;
var MAX_IDS = 25;
...
async function fetchGapQuotes(ids) {
  ...
  try {
    ...
    geckoFailed = true;
    console.warn("[crypto-quotes] CoinGecko gap fetch failed:", err.message);
  }
  ...
  try {
    ...
    console.warn("[crypto-quotes] CoinPaprika gap fill failed:", err.message);
  }
...
async function fetchGapQuotesViaRelay(ids) {
  ...
  try {
    ...
    const data = await resp.json();
    if (!Array.isArray(data.quotes)) return /* @__PURE__ */ new Map();
    const got = /* @__PURE__ */ new Map();
    ...
    console.warn("[crypto-quotes] relay gap fetch failed:", err.message);
    return /* @__PURE__ */ new Map();
  }
}
async function listCryptoQuotes(ctx, req) {
  ...
  let seedQuotes = [];
  try {
    const seedData = await getCachedJson(SEED_CACHE_KEY, true);
  ...
  if (ids.length === 0) {
    ...
    return {
      quotes: seedQuotes.map((q) => ({ ...q, change7d: q.change7d ?? 0 })),
      unresolvedIds: [],
      provider: "seed"
    };
  }
  ...
  const seedById = /* @__PURE__ */ new Map();
  for (const q of seedQuotes) {
    const metaId = SYMBOL_TO_ID.get(q.symbol);
    if (!metaId) continue;
    const quote = { ...q, change7d: q.change7d ?? 0 };
  ...
  let provider = gapIds.length === 0 ? "seed" : seedHits.length > 0 ? "mixed" : "upstream";
  if (gapIds.length > 0) {
    ...
    try {
      const cached = await cachedFetchJson(
        ...
        120,
        { timeoutMs: 15e3, cacheFetcherErrors: false }
      );
      if (cached) {
        for (const [id, quote] of Object.entries(cached)) resolved.set(id, quote);
      }
    } catch (err) {
      console.warn("[crypto-quotes] upstream gap fetch failed:", err.message);
    }
  ...
  const quotes = accepted.map((id) => resolved.get(id)).filter((quote) => Boolean(quote));
...
// server/worldmonitor/market/v1/list-commodity-quotes.ts
var BOOTSTRAP_KEY2 = "market:commodities-bootstrap:v1";
var MAX_SYMBOLS = 64;
var SUPPORTED_COMMODITY_SYMBOLS = new Set(
  commodities_default.commodities.map((c) => c.symbol)
);
function normalizeCommoditySymbol(raw) {
  return raw.trim().replace(/\s+/g, "").slice(0, 32).toUpperCase();
}
function throwUnsupportedCommodityError(symbols) {
...
function resolveCommodityQuery(rawSymbols, supported = SUPPORTED_COMMODITY_SYMBOLS, maxSymbols = MAX_SYMBOLS) {
  const symbols = [];
  const seen = /* @__PURE__ */ new Set();
  const unsupported = [];
...
function filterCommoditySeed(quotes, symbols) {
...
async function listCommodityQuotes(_ctx, req) {
  ...
  try {
    const bootstrap = await getCachedJson(BOOTSTRAP_KEY2, true);
...
init_redis();
var SEED_CACHE_KEY2 = "market:sectors:v2";
...
var import_stablecoin_classifier = __toESM(require_stablecoin_classifier(), 1);
init_redis();
...
// server/worldmonitor/market/v1/list-stablecoin-markets.ts
var SEED_CACHE_KEY3 = "market:stablecoins:v1";
var GAP_CACHE_KEY_PREFIX = "market:stablecoins:rpc:v1:";
var GAP_CACHE_TTL2 = 600;
var GAP_NEGATIVE_TTL = 300;
var GAP_DEGRADED_TTL = 60;
var GAP_FETCH_TIMEOUT_MS = 25e3;
var MAX_LOOKUP_COINS = 25;
var MAX_ECHOED_ID_LENGTH = 64;
var COINGECKO_ID_PATTERN = /^[a-z0-9][a-z0-9-]{0,63}$/;
var UNRESOLVED_REASONS = Object.freeze([
  "INVALID_ID",
  "OVER_CAP",
  "NOT_FOUND",
  "PROVIDER_ERROR"
]);
var DATA_STATUSES = Object.freeze(["OK", "PARTIAL", "UNAVAILABLE"]);
function unresolvedEntry(id, reason) {
  return { id, reason };
}
function isUsableStablecoin(value) {
  return Boolean(
    value && typeof value === "object" && typeof value.id === "string" && value.id.length > 0
  );
}
function isGapPayload(value) {
  if (!value || typeof value !== "object") return false;
  const payload = value;
  return Array.isArray(payload.coins) && (payload.source === "coingecko" || payload.source === "coinpaprika") && typeof payload.fetchedAt === "string";
}
function unavailableResponse(unresolved) {
  return {
...
function healthStatusFor(coinCount, depeggedCount) {
...
function normalizeRequestedCoins(raw) {
  const seen = /* @__PURE__ */ new Set();
  const lookupIds = [];
  const unresolved = [];
...
async function resolveGapCoins(ids) {
  const resolved = /* @__PURE__ */ new Map();
  if (ids.length === 0) return { resolved, providerFailed: false, fetchedAt: null };
  const cacheKey = `${GAP_CACHE_KEY_PREFIX}${await sha256Hex4([...ids].sort().join(","))}`;
  try {
    ...
    const payload = await cachedFetchJson(
      ...
      async () => {
        ...
        return { coins: usable.map((item) => (0, import_stablecoin_classifier.classifyStablecoin)(item)), source, fetchedAt: (/* @__PURE__ */ new Date()).toISOString() };
      },
      GAP_NEGATIVE_TTL,
    ...
    console.warn("[Stablecoin] gap lookup failed:", err.message);
...
async function listStablecoinMarkets(_ctx, req) {
  ...
  const seedRead = await readCachedJson(SEED_CACHE_KEY3, true);
  const seedUnreachable = seedRead.status === "error";
  const seed = seedRead.status === "hit" ? seedRead.value : null;
  ...
  const { resolved, providerFailed: lookupFailed, fetchedAt: gapFetchedAt } = await resolveGapCoins(gapIds);
  ...
  const dataStatus = unresolved.length === 0 ? "OK" : "PARTIAL";
  return {
    // ISO-8601 UTC sorts lexicographically, so the first is the oldest.
    timestamp: ages.sort()[0] ?? (/* @__PURE__ */ new Date()).toISOString(),
...
init_redis();
var SEED_CACHE_KEY4 = "market:etf-flows:v1";
var EMPTY_RESPONSE = {
  timestamp: (/* @__PURE__ */ new Date()).toISOString(),
  summary: {
    etfCount: 0,
...
async function listEtfFlows(_ctx, _req) {
  try {
    const seedData = await getCachedJson(SEED_CACHE_KEY4, true);
...
init_redis();
var COUNTRY_INDEX = openapi_filter_param_contracts_default.marketCountryStockIndexes;
var REDIS_CACHE_KEY = "market:stock-index:rpc:v1";
var RAILWAY_SEEDED_COUNTRY_INDEX_KEY_PREFIX = "market:stock-index:v1:";
var REDIS_CACHE_TTL = 1800;
var stockIndexCache = {};
var STOCK_INDEX_CACHE_TTL = 36e5;
function railwaySeededKey(code) {
  return `${RAILWAY_SEEDED_COUNTRY_INDEX_KEY_PREFIX}${code}`;
}
function isAvailableCountryStockIndex(value, code) {
  return Boolean(
    value && typeof value === "object" && value.available && value.code === code
  );
}
...
init_redis();
var SEED_CACHE_KEY5 = "market:gulf-quotes:v1";
async function listGulfQuotes(_ctx, _req) {
  try {
    const seedData = await getCachedJson(SEED_CACHE_KEY5, true);
...
var MODEL_FAILURE_THRESHOLD = 2;
var MODEL_QUARANTINE_MS = 10 * 6e4;
...
async function isProviderAvailable(apiUrl) {
  ...
  const cached = cache2.get(origin);
...
function modelKey(apiUrl, model) {
...
function readModelEntry(key) {
  const entry = modelCache.get(key);
  if (!entry) return void 0;
  const expiresAt = entry.quarantinedUntil > 0 ? entry.quarantinedUntil : entry.lastFailureAt + MODEL_QUARANTINE_MS;
...
function isModelRejection(status, body, model) {
  ...
  return patterns.some((pattern) => pattern.test(message2));
}
function isModelUsable(apiUrl, model) {
  const key = modelKey(apiUrl, model);
  if (!key) return true;
  const entry = readModelEntry(key);
  return !entry || entry.quarantinedUntil === 0;
}
function recordModelFailure(apiUrl, model, status, body) {
  ...
  if (entry.quarantinedUntil === 0 && entry.failures >= MODEL_FAILURE_THRESHOLD) {
    ...
    console.warn(
      `[llm-health] Model quarantined for ${MODEL_QUARANTINE_MS / 1e3}s: ${key} \u2014 rejected ${entry.failures}x with HTTP ${status}`
    );
  }
  modelCache.set(key, entry);
}
function recordModelSuccess(apiUrl, model) {
...
var CONTROL_CHARS_RE = /[\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F\xAD\u200B-\u200D\u2028\u2029\uFEFF]/g;
...
async function readBoundedErrorBody(resp, cap) {
  ...
  try {
    while (out.length < cap) {
      ...
      if (done) break;
      out += decoder2.decode(value, { stream: true });
    }
  } catch {
  } finally {
    try {
...
function findClosingChar(xmlData, char, i, errMsg2) {
  const closingIndex = xmlData.indexOf(char, i);
  if (closingIndex === -1) throw new Error(errMsg2);
  return closingIndex;
}
...
function providerSignal2(signal) {
  return signal ? AbortSignal.any([signal, AbortSignal.timeout(UPSTREAM_TIMEOUT_MS)]) : AbortSignal.timeout(UPSTREAM_TIMEOUT_MS);
}
async function searchWithExa(query, maxResults, days, apiKey, signal) {
...
async function searchWithBrave(query, maxResults, days, apiKey, signal) {
...
async function searchWithSerpApi(query, maxResults, days, apiKey, signal) {
...
async function searchViaProviders(query, maxResults, days, signal) {
  const providers = [
  ...
  for (const provider of providers) {
    ...
    for (const apiKey of candidates) {
      ...
      try {
        ...
        console.warn(`[stock-news-search] ${provider.id} failed: ${message2}`);
      }
    }
  }
  return null;
}
async function fetchGoogleNewsRss(query, maxResults, signal) {
...
async function searchRecentStockHeadlines(symbol, name, maxResults = 5, options = {}) {
  ...
  const cached = await cachedFetchJson(cacheKey, SEARCH_CACHE_TTL_SECONDS, async () => {
...
function inferDividendFrequency(paymentsPerYear) {
...
function paymentsPerYearFromInterval(sortedEntries, nowSec = Math.floor(Date.now() / 1e3), windowSec = 2 * 365.25 * 24 * 3600) {
  ...
  const medianGapDays = gaps.length % 2 === 1 ? gaps[mid] : (gaps[mid - 1] + gaps[mid]) / 2;
  if (medianGapDays <= 0) return 0;
  return 365.25 / medianGapDays;
}
function computeDividendCagr(annualTotals) {
...
async function fetchDividendProfile(symbol, currentPrice) {
  try {
    await yahooGate();
    ...
    const entries = Object.values(dividendEvents).filter((d) => typeof d.amount === "number" && d.amount > 0 && typeof d.date === "number").sort((a, b) => (a.date ?? 0) - (b.date ?? 0));
    if (entries.length === 0) return EMPTY_DIVIDEND_PROFILE;
    const annualTotals = /* @__PURE__ */ new Map();
    for (const entry of entries) {
      const year2 = new Date((entry.date ?? 0) * 1e3).getFullYear();
      annualTotals.set(year2, (annualTotals.get(year2) ?? 0) + (entry.amount ?? 0));
    }
    const now = Date.now();
    const oneYearAgo = now - 365.25 * 24 * 3600 * 1e3;
    const recentDivs = entries.filter((d) => (d.date ?? 0) * 1e3 >= oneYearAgo);
    ...
    if (recentDivs.length >= 3) {
      ...
      paymentsPerYear = byInterval > 0 ? byInterval : recentDivs.length || entries.length / Math.max(1, annualTotals.size);
    } else if (recentDivs.length > 0) {
      const lastTwo = entries.slice(-2);
      const mostRecentGapDays = lastTwo.length === 2 ? ((lastTwo[1].date ?? 0) - (lastTwo[0].date ?? 0)) / (24 * 3600) : Number.POSITIVE_INFINITY;
      ...
      if (byInterval > 0) {
        ...
        const daysSinceLast = (now / 1e3 - (entries[entries.length - 1].date ?? 0)) / (24 * 3600);
        if (daysSinceLast <= expectedGapDays * 1.5) {
          ...
          const indicatedAnnual = entries.slice(-indicatedPayments).reduce((sum, e) => sum + (e.amount ?? 0), 0);
    ...
    const exDividendDate = (latestEntry.date ?? 0) * 1e3;
...
var STOCK_ANALYSIS_ENGINE_VERSION = "v3-composite";
...
function computeRealizedVolatility(closes) {
  const logReturns = [];
  for (let i = 1; i < closes.length; i++) {
    const prev = closes[i - 1];
    const curr = closes[i];
    if (prev === void 0 || curr === void 0 || prev <= 0 || curr <= 0) continue;
...
function computeAtr(highs, lows, closes, period = ATR_PERIOD) {
  ...
  for (let i = 0; i < length; i++) {
    const high = highs[i];
    const low = lows[i];
    if (high === void 0 || low === void 0) continue;
    const prevClose = i > 0 ? closes[i - 1] : void 0;
    if (i === 0 || prevClose === void 0) {
...
var ET_PARTS_FMT = new Intl.DateTimeFormat("en-US", {
...
function etDow(year2, month, day2) {
  return new Date(Date.UTC(year2, month - 1, day2)).getUTCDay();
}
function nthWeekdayOfMonth(year2, month, weekday, n) {
  return 1 + (weekday - etDow(year2, month, 1) + 7) % 7 + (n - 1) * 7;
}
function shiftDays(year2, month, day2, delta) {
  const d = new Date(Date.UTC(year2, month - 1, day2 + delta));
  return { month: d.getUTCMonth() + 1, day: d.getUTCDate() };
}
function observedHoliday(year2, month, day2) {
  const w = etDow(year2, month, day2);
  if (w === 6) return shiftDays(year2, month, day2, -1);
  if (w === 0) return shiftDays(year2, month, day2, 1);
  return { month, day: day2 };
}
function isNyseFullHoliday(year2, month, day2) {
  const key = month * 100 + day2;
  const holidays = /* @__PURE__ */ new Set();
  const nyDow = etDow(year2, 1, 1);
  if (nyDow === 0) holidays.add(102);
  else if (nyDow !== 6) holidays.add(101);
  holidays.add(100 + nthWeekdayOfMonth(year2, 1, 1, 3));
  holidays.add(200 + nthWeekdayOfMonth(year2, 2, 1, 3));
  ...
  const gf = shiftDays(year2, easterMonth, easterDay, -2);
  holidays.add(gf.month * 100 + gf.day);
  const lastDayMay = new Date(Date.UTC(year2, 5, 0)).getUTCDate();
  holidays.add(500 + lastDayMay - (etDow(year2, 5, lastDayMay) - 1 + 7) % 7);
  const jt = observedHoliday(year2, 6, 19);
  holidays.add(jt.month * 100 + jt.day);
  const ind = observedHoliday(year2, 7, 4);
  holidays.add(ind.month * 100 + ind.day);
  holidays.add(900 + nthWeekdayOfMonth(year2, 9, 1, 1));
  holidays.add(1100 + nthWeekdayOfMonth(year2, 11, 4, 4));
  const xmas = observedHoliday(year2, 12, 25);
  holidays.add(xmas.month * 100 + xmas.day);
  return holidays.has(key);
}
function isNyseEarlyCloseDay(year2, month, day2) {
  const w = etDow(year2, month, day2);
  if (w === 0 || w === 6 || isNyseFullHoliday(year2, month, day2)) return false;
  if (month === 7 && day2 === 3) return true;
  if (month === 12 && day2 === 24) return true;
  if (month === 11 && day2 === nthWeekdayOfMonth(year2, 11, 4, 4) + 1) return true;
  return false;
}
function getUsEquitySessionAt(date = /* @__PURE__ */ new Date()) {
  const parts = {};
  for (const part of ET_PARTS_FMT.formatToParts(date)) parts[part.type] = part.value;
  const year2 = Number(parts.year), month = Number(parts.month), day2 = Number(parts.day);
  if (parts.weekday === "Sat" || parts.weekday === "Sun") return "closed";
  if (isNyseFullHoliday(year2, month, day2)) return "closed";
  const early = isNyseEarlyCloseDay(year2, month, day2);
...
function usEquityHoursApply(symbol, currency) {
  return currency === "USD" && !/\.[A-Za-z]+$/.test(symbol);
}
async function fetchExtendedHoursQuote(symbol, session) {
  try {
    await yahooGate();
    ...
    const lastRegularClose = [result?.meta?.regularMarketPrice, result?.meta?.chartPreviousClose].find((value) => typeof value === "number" && Number.isFinite(value) && value > 0);
    if (lastRegularClose === void 0) return null;
...
function optionalFinite(field) {
  const raw = field?.raw;
  return typeof raw === "number" && Number.isFinite(raw) ? raw : void 0;
}
function optionalYahooDebtToEquity(field) {
  const raw = optionalFinite(field);
  return raw === void 0 ? void 0 : raw / 100;
}
function optionalFinancialCurrency(value) {
  if (typeof value !== "string") return void 0;
  const normalized = value.trim().toUpperCase();
  return /^[A-Z]{3}$/.test(normalized) ? normalized : void 0;
}
var EMPTY_ANALYST_DATA = {
...
async function fetchYahooAnalystData(symbol) {
  try {
    ...
    const analystConsensus = currentPeriod ? {
      ...
      total: (typeof currentPeriod.strongBuy === "number" ? currentPeriod.strongBuy : 0) + (typeof currentPeriod.buy === "number" ? currentPeriod.buy : 0) + (typeof currentPeriod.hold === "number" ? currentPeriod.hold : 0) + (typeof currentPeriod.sell === "number" ? currentPeriod.sell : 0) + (typeof currentPeriod.strongSell === "number" ? currentPeriod.strongSell : 0),
    ...
    const recentUpgrades = rawHistory.slice(0, 5).map((entry) => ({
...
var EARNINGS_CALENDAR_CACHE_KEY = "market:earnings-calendar:v1";
function selectEarningsForSymbol(earnings, symbol, todayStr) {
  let pick = null;
  ...
  if (!pick) return null;
  const result = { nextEarningsDate: pick.date };
...
async function fetchUpcomingEarnings() {
  try {
    const cached = await getCachedJson(EARNINGS_CALENDAR_CACHE_KEY, true);
    return cached?.earnings ?? [];
  } catch {
    return [];
  }
}
function deriveSignal(score, trendStatus) {
...
var FUNDAMENTAL_TECHNICAL_WEIGHT = 0.65;
function computeFundamentalScore(f) {
  const finite = (v) => typeof v === "number" && Number.isFinite(v) ? v : null;
  const quality = [];
  const profitMargin = finite(f.profitMargin);
  if (profitMargin !== null) quality.push(bandScore(profitMargin, [[-Infinity, 15], [0, 42], [0.05, 58], [0.12, 75], [0.2, 90]]));
  const operatingMargin = finite(f.operatingMargin);
  if (operatingMargin !== null) quality.push(bandScore(operatingMargin, [[-Infinity, 18], [0, 45], [0.08, 62], [0.18, 80], [0.3, 92]]));
  const grossMargin = finite(f.grossMargin);
  if (grossMargin !== null) quality.push(bandScore(grossMargin, [[-Infinity, 30], [0.2, 50], [0.35, 64], [0.5, 78], [0.65, 90]]));
  const returnOnEquity = finite(f.returnOnEquity);
  if (returnOnEquity !== null) quality.push(bandScore(returnOnEquity, [[-Infinity, 20], [0, 45], [0.1, 65], [0.18, 80], [0.3, 90]]));
  const returnOnAssets = finite(f.returnOnAssets);
  if (returnOnAssets !== null) quality.push(bandScore(returnOnAssets, [[-Infinity, 20], [0, 45], [0.05, 63], [0.1, 80], [0.18, 90]]));
  const freeCashflow = finite(f.freeCashflow);
  if (freeCashflow !== null) quality.push(freeCashflow > 0 ? 72 : 32);
  const growth = [];
  ...
  if (earningsGrowth !== null) growth.push(bandScore(earningsGrowth, [[-Infinity, 12], [-0.1, 35], [0.05, 58], [0.2, 78], [0.4, 90]]));
  const leverage = [];
  const debtToEquity = finite(f.debtToEquity);
  if (debtToEquity !== null) {
    leverage.push(debtToEquity < 0 ? 25 : debtToEquity <= 0.3 ? 90 : debtToEquity <= 0.6 ? 78 : debtToEquity <= 1 ? 62 : debtToEquity <= 2 ? 42 : 22);
  }
...
function computeCompositeScore(signalScore, fundamentalScore) {
...
function getFallbackOverlay(name, technical, headlines, ratingScore = technical.signalScore, ratingSignal = technical.signal) {
...
function getRatingFallbackOverlay(name, technical, headlines, fundamentalScore, compositeScore, ratingSignal) {
  ...
  return {
    ...
    bullishFactors: fundamentalScore >= 55 ? [fundamentalFactor, ...overlay.bullishFactors].slice(0, 4) : overlay.bullishFactors,
    riskFactors: fundamentalScore < 55 ? [fundamentalFactor, ...overlay.riskFactors].slice(0, 4) : overlay.riskFactors
  };
}
async function buildAiOverlay(symbol, name, technical, headlines, fundamentals, fundamentalScore, compositeScore, ratingSignal) {
  ...
  const llm = await callLlm({
      ...
      },
      {
        ...
        content: JSON.stringify({
          ...
          rating: {
            signal: ratingSignal,
            compositeScore,
            fundamentalScore: fundamentalScore ?? void 0
          },
          technical: {
          ...
          fundamentals: hasFundamentals ? fundamentals : void 0
        })
      }
    ],
    temperature: 0.2,
    ...
    validate: (content) => {
      try {
        ...
        return typeof technicalNarrative === "object" && technicalNarrative !== null && typeof technicalNarrative.summary === "string" && typeof technicalNarrative.action === "string" && typeof ratingNarrative === "object" && ratingNarrative !== null && typeof ratingNarrative.summary === "string" && typeof ratingNarrative.action === "string" && (headlines.length === 0 || typeof parsed.newsSentiment === "number" && Number.isFinite(parsed.newsSentiment));
      } catch {
        return false;
      }
    }
  ...
  try {
    ...
    const mergeWithFallback = (narrative, fallback) => ({
      ...
      bullishFactors: Array.isArray(narrative?.bullishFactors) ? narrative.bullishFactors.filter((factor) => typeof factor === "string").slice(0, 4) : fallback.bullishFactors,
      riskFactors: Array.isArray(narrative?.riskFactors) ? narrative.riskFactors.filter((factor) => typeof factor === "string").slice(0, 4) : fallback.riskFactors,
...
async function analyzeStock(_ctx, req, options = {}) {
  ...
  const fetchFreshAnalysis = async () => {
    ...
    const marketSession = usEquityHoursApply(symbol, history.currency || "USD") ? getUsEquitySessionAt(options.now) : "";
    const [headlines, dividend, extendedQuote, earningsCalendar] = await Promise.all([
      ...
      marketSession === "pre" || marketSession === "post" ? fetchExtendedHoursQuote(symbol, marketSession) : Promise.resolve(null),
    ...
    const earnings = selectEarningsForSymbol(earningsCalendar, symbol, (options.now ?? /* @__PURE__ */ new Date()).toISOString().slice(0, 10));
    const response = buildAnalysisResponse({
      ...
      dividend,
      marketSession,
      extended: extendedQuote ?? void 0,
      earnings
    });
    await storeStockAnalysisSnapshot(response, includeNews);
    return response;
  };
  const cached = options.now ? await fetchFreshAnalysis() : await cachedFetchJson(cacheKey, CACHE_TTL_SECONDS2, fetchFreshAnalysis, void 0, {
...
init_redis();
var CACHE_TTL_SECONDS3 = 900;
var DEFAULT_WINDOW_DAYS = 10;
var MIN_REQUIRED_BARS = 80;
var MAX_EVALUATIONS = 8;
var MIN_ANALYSIS_BARS = 60;
var STOCK_BACKTEST_ENGINE_VERSION = "v3-technical-only";
var STOCK_BACKTEST_RATING_BASIS = "technical_only";
...
var backtestStock = async (_ctx, req) => {
  ...
  try {
    const cached = await cachedFetchJson(cacheKey, CACHE_TTL_SECONDS3, async () => {
      ...
      const response = {
        ...
        summary: `Validated ${actionableEvaluations} technical-only signal records over ${evalWindowDays} trading days with ${round2(winRate)}% win rate and ${round2(avgSimulatedReturnPct)}% average simulated return. Point-in-time fundamentals are not included.`,
        generatedAt: (/* @__PURE__ */ new Date()).toISOString(),
...
init_redis();
var SEED_CACHE_KEY6 = "market:crypto-sectors:v1";
async function listCryptoSectors(_ctx, _req) {
  try {
    const seedData = await getCachedJson(SEED_CACHE_KEY6, true);
...
init_redis();
var SEED_CACHE_KEY7 = "market:defi-tokens:v1";
async function listDefiTokens(ctx, _req) {
  try {
    const seedData = await getCachedJson(SEED_CACHE_KEY7, true);
    if (!seedData?.tokens?.length) return markNoStoreFallbackResponse(ctx.request, { tokens: [] });
    const tokens = seedData.tokens.map((t) => ({
      name: t.name,
      symbol: t.symbol,
      price: t.price,
      change: t.change24h,
...
init_redis();
var SEED_CACHE_KEY8 = "market:ai-tokens:v1";
async function listAiTokens(ctx, _req) {
  try {
    const seedData = await getCachedJson(SEED_CACHE_KEY8, true);
    if (!seedData?.tokens?.length) return markNoStoreFallbackResponse(ctx.request, { tokens: [] });
    const tokens = seedData.tokens.map((t) => ({
      name: t.name,
      symbol: t.symbol,
...
init_redis();
var SEED_CACHE_KEY9 = "market:other-tokens:v1";
async function listOtherTokens(ctx, _req) {
  try {
    const seedData = await getCachedJson(SEED_CACHE_KEY9, true);
    if (!seedData?.tokens?.length) return markNoStoreFallbackResponse(ctx.request, { tokens: [] });
    const tokens = seedData.tokens.map((t) => ({
      name: t.name,
      symbol: t.symbol,
...
init_redis();
var SEED_CACHE_KEY10 = "market:fear-greed:v1";
async function getFearGreedIndex(_ctx, _req) {
  try {
    const raw = await getCachedJson(SEED_CACHE_KEY10, true);
    if (!raw?.composite) return { compositeScore: 0, compositeLabel: "", unavailable: true };
    const comp = raw.composite;
    const cats = raw.categories ?? {};
    const hdr = raw.headerMetrics ?? {};
    const mapCat = (c) => ({
    ...
    const rawSectors = raw.sectorPerformance ?? [];
    const sectorPerformance = rawSectors.map((s) => {
    ...
    return { compositeScore: 0, compositeLabel: "", unavailable: true };
  }
}

// server/worldmonitor/market/v1/list-earnings-calendar.ts
init_redis();
var SEED_CACHE_KEY11 = "market:earnings-calendar:v1";
async function listEarningsCalendar(_ctx, _req) {
  try {
    const cached = await getCachedJson(SEED_CACHE_KEY11, true);
    if (!cached?.earnings?.length) {
      return { earnings: [], fromDate: "", toDate: "", total: 0, unavailable: true };
    }
    const entries = cached.earnings.map((e) => ({
      symbol: e.symbol ?? "",
      company: e.company ?? "",
      date: e.date ?? "",
      hour: e.hour ?? "",
...
init_redis();
var SEED_CACHE_KEY12 = "market:cot:v1";
function mapCotPositioning(raw) {
  if (!raw?.instruments || raw.instruments.length === 0) {
    return { instruments: [], reportDate: "", unavailable: true };
  }
  const instruments = raw.instruments.map((item) => ({
...
async function getCotPositioning(_ctx, _req) {
  try {
    const raw = await getCachedJson(SEED_CACHE_KEY12, true);
...
init_redis();
var CACHE_TTL_SECONDS4 = 86400;
var SIX_MONTHS_MS = 6 * 30 * 24 * 60 * 60 * 1e3;
var PURCHASE_CODES = /* @__PURE__ */ new Set(["P"]);
var SALE_CODES = /* @__PURE__ */ new Set(["S"]);
var NEUTRAL_CODES = /* @__PURE__ */ new Set(["M", "A", "D", "F"]);
async function getInsiderTransactions(_ctx, req) {
  const symbol = sanitizeSymbol(req.symbol);
  if (!symbol) {
  ...
  try {
    const result = await cachedFetchJson(cacheKey, CACHE_TTL_SECONDS4, async () => {
      await finnhubGate();
      ...
      const mapped = recent.filter(
        (tx) => PURCHASE_CODES.has(tx.transactionCode) || SALE_CODES.has(tx.transactionCode) || NEUTRAL_CODES.has(tx.transactionCode)
      ).sort((a, b) => new Date(b.transactionDate).getTime() - new Date(a.transactionDate).getTime()).slice(0, 20).map((tx) => ({
        name: String(tx.name ?? ""),
        ...
        // exclude M rows.
        value: NEUTRAL_CODES.has(tx.transactionCode) ? 0 : Math.abs((tx.change ?? 0) * (tx.transactionPrice ?? 0)),
...
init_redis();
var SEED_CACHE_KEY13 = "market:breadth-history:v1";
...
async function getMarketBreadthHistory(_ctx, _req) {
  try {
    const raw = await getCachedJson(SEED_CACHE_KEY13, true);
...
init_redis();
var COMMODITY_KEY = "market:commodities-bootstrap:v1";
var COT_KEY = "market:cot:v1";
var GOLD_EXTENDED_KEY = "market:gold-extended:v1";
var GOLD_ETF_FLOWS_KEY = "market:gold-etf-flows:v1";
var GOLD_CB_RESERVES_KEY = "market:gold-cb-reserves:v1";
var XAU_FX = [
...
function mapCot(raw) {
  ...
  const managedMoney = raw.managedMoney ? mapCategory(raw.managedMoney) : mapCategory({
  ...
  const producerSwap = raw.producerSwap ? mapCategory(raw.producerSwap) : mapCategory({
...
async function getGoldIntelligence(_ctx, _req) {
  try {
    const [rawPayload, rawCot, rawExtended, rawEtfFlows, rawCbReserves] = await Promise.all([
      getCachedJson(COMMODITY_KEY, true),
      getCachedJson(COT_KEY, true),
      getCachedJson(GOLD_EXTENDED_KEY, true),
      getCachedJson(GOLD_ETF_FLOWS_KEY, true),
      getCachedJson(GOLD_CB_RESERVES_KEY, true)
    ]);
    ...
    const goldSilverRatio = goldPrice > 0 && silverPrice > 0 ? goldPrice / silverPrice : void 0;
    const goldPlatinumPremiumPct = goldPrice > 0 && platinumPrice > 0 ? (goldPrice - platinumPrice) / platinumPrice * 100 : void 0;
    const crossCurrencyPrices = [];
    if (goldPrice > 0) {
    ...
    const session = goldExt ? { dayHigh: goldExt.dayHigh, dayLow: goldExt.dayLow, prevClose: goldExt.prevClose } : void 0;
    const returns = goldExt ? { ...goldExt.returns } : void 0;
    const range52w = goldExt ? { ...goldExt.range52w } : void 0;
    const drivers = (rawExtended?.drivers ?? []).map((d) => ({
      symbol: d.symbol,
      label: d.label,
      value: d.value,
    ...
    const cbReserves = rawCbReserves && Array.isArray(rawCbReserves.topHolders) && rawCbReserves.topHolders.length >= 5 ? {
      asOfMonth: rawCbReserves.asOfMonth,
      totalTonnes: rawCbReserves.totalTonnes,
      topHolders: rawCbReserves.topHolders.map((h) => ({
        iso3: h.iso3,
        name: h.name,
        tonnes: h.tonnes,
        pctOfReserves: h.pctOfReserves
      })),
      topBuyers12m: (rawCbReserves.topBuyers12m ?? []).map((m) => ({
        iso3: m.iso3,
        name: m.name,
        deltaTonnes12m: m.deltaTonnes12m
      })),
      topSellers12m: (rawCbReserves.topSellers12m ?? []).map((m) => ({
        iso3: m.iso3,
        name: m.name,
        deltaTonnes12m: m.deltaTonnes12m
      }))
    } : void 0;
    const etfFlows = rawEtfFlows && Number.isFinite(rawEtfFlows.tonnes) && rawEtfFlows.tonnes > 0 ? {
...
init_redis();
var SEED_CACHE_KEY14 = "market:hyperliquid:flow:v1";
function numToStr(v) {
...
async function getHyperliquidFlow(_ctx, _req) {
  try {
    const raw = await getCachedJson(SEED_CACHE_KEY14, true);
