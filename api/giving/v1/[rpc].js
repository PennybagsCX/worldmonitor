...
var giving_published_estimate_claims_default = [
  {
...
// server/worldmonitor/giving/v1/published-estimates.ts
var GIVING_PROVENANCE_STATUSES = [
  "verified",
  "partially_verified",
  "unverified",
  "not_collected",
...
function normalizeGivingStatus(status) {
  return GIVING_PROVENANCE_STATUSES.includes(status) ? status : "unverified";
}
var PUBLISHED_ESTIMATE_CLAIMS = giving_published_estimate_claims_default;
function annualizedPlatformValueUsd(claim) {
  if (!claim.platform || !claim.includedInHighlightedAggregate || normalizeGivingStatus(claim.status) !== "verified" || claim.reportedUnit !== "USD" || !Number.isFinite(claim.reportedValue) || claim.reportedValue <= 0) {
    return void 0;
  }
...
function verifiedPublicationDate(claims, platform) {
  return claims.find((claim) => claim.platform === platform && normalizeGivingStatus(claim.status) === "verified" && claim.sourcePublishedAt.trim())?.sourcePublishedAt ?? "";
}
function platformProjection(claims, platform, dataFreshness) {
  const annualizedUsd = claims.filter((claim) => claim.platform === platform).reduce((total, claim) => total + (annualizedPlatformValueUsd(claim) ?? 0), 0);
  return {
    platform,
...
function verifiedContextValue(claims, metric) {
  const claim = claims.find((entry) => entry.contextMetric === metric);
  return claim && normalizeGivingStatus(claim.status) === "verified" ? claim.reportedValue : 0;
}
function categoryProjections() {
  return [
    "Medical & Health",
    "Disaster Relief",
    "Education",
...
function publicProvenance(claim) {
  return {
...
function dataMode(claims) {
  return claims.some((claim) => {
    const status = normalizeGivingStatus(claim.status);
    return status === "unverified" || status === "partially_verified";
  }) ? "partial_estimate" : "published_estimate";
}
function buildPublishedEstimateSummary(claims = PUBLISHED_ESTIMATE_CLAIMS, generatedAt = (/* @__PURE__ */ new Date()).toISOString()) {
  const platforms = [
...
// server/worldmonitor/giving/v1/get-giving-summary.ts
var REDIS_CACHE_KEY = "giving:summary:v2";
var REDIS_CACHE_TTL = 3600;
function responseFetchedAt(summary) {
...
async function getGivingSummary(_ctx, req) {
  try {
    const result = await cachedFetchJson(REDIS_CACHE_KEY, REDIS_CACHE_TTL, async () => {
      const summary2 = buildPublishedEstimateSummary();
      return { summary: summary2, fetchedAt: responseFetchedAt(summary2), dataAvailable: true };
    });
    if (!result) {
      return { summary: void 0, fetchedAt: 0, dataAvailable: false };
    }
