...
async function validateSessionToken(token) {
  ...
  const body = tail.slice(0, dot2);
  const sig = tail.slice(dot2 + 1);
...
function markRetryableResponse(req) {
...
var MAX_SUMMARY_HEADLINES = 5;
var MAX_GEO_CONTEXT_LEN = 2e3;
var MAX_BODY_LEN = 400;
function selectUniqueHeadlinePairs(pairs) {
  const selected = [];
...
function canonicalizeSummaryInputs(headlines, geoContext, bodies) {
  ...
  const canonBodies = canonHeadlines.map((_, i) => {
    const b = rawBodies[i];
...
function buildSummaryCacheKey(headlines, mode, geoContext, variant, lang, systemAppend, bodies) {
  ...
  const bodiesHash = anyBody ? ":bd" + hashString(topPairs.map((p) => `${p.b.length}:${p.b}`).join("|")) : "";
...
var DIM = 512;
var STORY_SIMILARITY_THRESHOLD = 0.615;
var WEIGHT_TOKEN = 2;
var WEIGHT_BIGRAM = 1.5;
var WEIGHT_CHARGRAM = 1;
var BOOST_ENTITY = 3;
var BOOST_NUMBER = 2;
...
var MAX_IDENTITY_CHARS = 300;
function stripAttributionSuffix(text) {
  let out = text || "";
...
function contentTokens(text) {
  ...
  for (const raw of clamped.split(/\s+/)) {
    ...
    const capitalized = new RegExp("^\\p{Lu}", "u").test(clean);
    const hasDigit = new RegExp("\\p{N}", "u").test(clean);
...
function addFeature(vec, feature, weight) {
  ...
  const sign = (fnv1a(feature, 2654435769) & 1) === 1 ? 1 : -1;
  vec[idx] += sign * weight;
}
...
var activeVectorizer = lexicalStoryVector;
function storyVector(text) {
...
var CONTAINMENT_RESCUE_MIN_TOKENS = 4;
var CONTAINMENT_RESCUE_RATIO = 0.9;
var CONTAINMENT_RESCUE_SCORE = 0.9;
...
var MAX_CANDIDATE_BUCKET = 250;
function clusterTexts(texts, opts = {}) {
  ...
  const byExactText = /* @__PURE__ */ new Map();
  ...
  for (let i = 0; i < texts.length; i++) {
    ...
    if (first === void 0) byExactText.set(normalized, i);
    else union(first, i);
  }
  for (let i = 0; i < texts.length; i++) {
  ...
  return Array.from(byRoot.entries()).sort((a, b) => a[0] - b[0]).map(([, members]) => members);
}
...
var MIN_CORROBORATING_PUBLISHERS = 2;
...
async function assignStoryIdentity(items, normalizeTitle2, sha256Hex5) {
  ...
  await Promise.all(clusters.map(async (indices) => {
    ...
    for (const i of indices) {
      const normalized = normalizeTitle2(items[i].title || "");
      if (!normalized) continue;
      const publishedAt = typeof items[i].publishedAt === "number" && Number.isFinite(items[i].publishedAt) ? items[i].publishedAt : Infinity;
      if (canonical === null || publishedAt < canonicalPublishedAt || publishedAt === canonicalPublishedAt && normalized < canonical) {
    ...
    if (canonical === null) {
      await Promise.all(indices.map(async (i) => {
        const titleHash2 = await sha256Hex5(`untrackable:${items[i].source || ""}:${items[i].title || ""}`);
        assignment.set(items[i], { titleHash: titleHash2, corroborationCount: 1, memberTitleHashes: [] });
      }));
      return;
    }
    const titleHash = await sha256Hex5(canonical);
    const memberNormalized = /* @__PURE__ */ new Set();
    for (const i of indices) {
      const normalized = normalizeTitle2(items[i].title || "");
      if (normalized) memberNormalized.add(normalized);
    }
    const memberTitleHashes = await Promise.all([...memberNormalized].map((n) => sha256Hex5(n)));
    for (const i of indices) {
...
function adoptExistingCanonical(memberTitleHashes, defaultHash, aliasTargetByHash) {
  const counts = /* @__PURE__ */ new Map();
  for (const memberHash of Array.isArray(memberTitleHashes) ? memberTitleHashes : []) {
    const target = aliasTargetByHash instanceof Map ? aliasTargetByHash.get(memberHash) : aliasTargetByHash?.[memberHash];
...
function sanitizeForPromptLine(input) {
...
function sanitizeHeadline(input) {
  if (typeof input !== "string") return "";
...
function sanitizeHeadlinesLight(headlines) {
...
var CACHE_TTL_SECONDS2 = 86400;
var MAX_BODY_INTERPOLATION_LEN = 400;
function buildArticlePrompts(headlines, uniqueHeadlines, opts) {
  const interpolateBodies = opts.mode !== "translate" && Array.isArray(opts.bodies) && opts.bodies.some((b) => typeof b === "string" && b.length > 0);
  const headlineText = interpolateBodies ? uniqueHeadlines.map((h, i) => {
    const rawBody = opts.bodies?.[i] ?? "";
    const clipped = rawBody.slice(0, MAX_BODY_INTERPOLATION_LEN);
    return clipped.length > 0 ? `${i + 1}. ${h}
    Context: ${clipped}` : `${i + 1}. ${h}`;
...
function getPremiumRpcBillingErrorType(denial) {
...
async function summarizeArticle(ctx, req) {
  ...
  const MAX_BODY_LEN2 = 400;
  const rawBodiesIn = Array.isArray(req.bodies) ? req.bodies : [];
  const intakePairs = (req.headlines || []).slice(0, MAX_HEADLINES).map((h, i) => ({
    h: typeof h === "string" ? h.slice(0, MAX_HEADLINE_LEN2) : "",
    b: rawBodiesIn[i]
  })).map((p) => ({ h: sanitizeHeadlinesLight([p.h])[0] ?? "", b: p.b })).filter((p) => p.h.length > 0);
  const headlines = intakePairs.map((p) => p.h);
  const sanitizedGeoContext = sanitizeForPrompt(
    typeof geoContext === "string" ? geoContext.slice(0, MAX_GEO_CONTEXT_LEN2) : ""
  );
  const bodies = intakePairs.map(
    (p) => typeof p.b === "string" ? sanitizeForPrompt(p.b.slice(0, MAX_BODY_LEN2)) : ""
  );
  ...
  try {
    const cacheKey = buildSummaryCacheKey(headlines, mode, sanitizedGeoContext, variant, lang, systemAppend || void 0, bodies);
    const { data: result, source } = await cachedFetchJsonWithMeta(
      ...
      async () => {
        ...
        const aliasWinners = /* @__PURE__ */ new Map();
        for (const pair of nonEmpty) {
          const current = aliasWinners.get(pair.h);
          if (!current || pair.keyH < current.keyH || pair.keyH === current.keyH && pair.b < current.b) {
        ...
        const effectiveSystemPrompt = sanitizedAppend ? `${systemPrompt}

---

${sanitizedAppend}` : systemPrompt;
        ...
        const rawText = typeof message2?.content === "string" ? message2.content.trim() : "";
...
init_redis();
var CACHE_KEY_PATTERN = new RegExp(openapi_filter_param_contracts_default.newsSummarizeArticleCacheKeyPattern);
var CURRENT_NAMESPACE = `summary:${CACHE_VERSION}:`;
...
var HISTORICAL_ANCHORED_PREFIX_RE = /^(?:science history|throwback|flashback)\s*:?/i;
var HISTORICAL_BRAND_PREFIX_RE = /^(?:[A-Z][\w'&-]*\s+){1,4}(?:[Tt]hrowback|[Ff]lashback)(?:\s+[A-Za-z]+)?\s*:/;
var HISTORICAL_PREFIX_WITH_YEAR_RE = /^on this day in\s+(?:19|20)\d{2}\b/i;
var THIS_DAY_IN_HISTORY_RE = /^this day in history\b/i;
var HISTORICAL_PHRASE_RE = /\b(?:\d+\s+(?:years?|decades?|months?)\s+(?:ago|after|later)|anniversary|in memoriam|remembering|remembered|commemorat(?:e|es|ed|ion)|retrospective)\b/i;
var FULL_DATE_RE = /\b(?:january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{1,2},?\s+((?:19|20)\d{2})\b/i;
var ISO_DATE_RE = /\b((?:19|20)\d{2})-\d{1,2}-\d{1,2}\b/;
function isPastRetrospectiveYear(year2, nowMs) {
  const currentYear = new Date(nowMs).getUTCFullYear();
  return year2 < currentYear - 1;
}
function hasHistoricalMarker(title, nowMs = Date.now()) {
...
// server/_shared/opinion-classifier.js
var STRONG_URL_SEGMENTS = [
  "/opinion/",
  "/opinions/",
  "/views/",
  "/commentary/",
...
var STRONG_HEADLINE_PREFIX_RE = /^(?:opinion|analysis|commentary|op-?ed|editorial|perspective|viewpoint)\s*:/i;
var COMMENTARY_HOSTNAMES = /* @__PURE__ */ new Set([
  "thebulletin.org",
  // Bulletin of the Atomic Scientists — entirely commentary/analysis
  "project-syndicate.org",
  // Project Syndicate — op-eds from world leaders / academics
  "foreignaffairs.com",
  // Foreign Affairs — CFR's analysis quarterly; long-form essays
  "warontherocks.com"
  // War on the Rocks — defense analysis blog
...
var CORROBORATING_DESCRIPTION_RE = /\b(?:columnist|op-?ed|opinion piece|our columnist|argues that|posits that|makes the case|the case for|guest essay|editorial board)\b/i;
var HISTORICAL_EXPLAINER_HEADLINE_RE = /^(?:how|why)\b[\s\S]{0,180}\b(?:changed|(?:re)?shaped|transformed|altered|became|remade|defined)\b/i;
var HISTORICAL_ANNIVERSARY_HEADLINE_RE = /^(?:(?:\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\s+(?:years?|decades?)|a\s+decade)\s+(?:on(?:\s+from)?|after|since)\b/i;
var HISTORICAL_ANNIVERSARY_CONTEXT_RE = /\b(?:legacy|(?:lasting|long[-\s]?term)\s+(?:impact|effects?|consequences?))\b/i;
var HISTORICAL_EXPLAINER_TITLE_TIME_RE = /\b(?:(?:\d+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\s+(?:years?|decades?)\s+(?:ago|after|later|on(?:\s+from)?|since)|anniversary|retrospective|(?:a|this)\s+look back)\b/i;
var HISTORICAL_EXPLAINER_DESCRIPTION_LOOKBACK_RE = /\b(?:a|this)\s+look[-\s]?back\b/i;
var HISTORICAL_EXPLAINER_LIVE_EVENT_RE = /\b(?:today|overnight|this morning|an?\s+hour ago|hours ago|breaking)\b/i;
var HISTORICAL_EVENT_YEAR_RE = /\b((?:19|20)\d{2})\s+(?:coup(?:\s+attempt)?|war|invasion|election|referendum|uprising|protests?|crackdown|attack|crisis|conflict|earthquake|disaster)\b/gi;
function publishedYear(publishedAt) {
  if (typeof publishedAt !== "number" && typeof publishedAt !== "string") return null;
  const timestamp = typeof publishedAt === "string" && /^\d+$/.test(publishedAt) ? Number(publishedAt) : publishedAt;
  const date = new Date(timestamp);
  const year2 = date.getUTCFullYear();
  return Number.isNaN(date.getTime()) ? null : year2;
}
...
function isHistoricalExplainer(title, description, publishedAt) {
  ...
  if (HISTORICAL_ANNIVERSARY_HEADLINE_RE.test(headline) && HISTORICAL_ANNIVERSARY_CONTEXT_RE.test(fullText)) return true;
...
function classifyOpinion(story) {
...
// server/_shared/feelgood-classifier.js
var HARD_NEWS_VETO_RE = /\b(?:ceasefire|hostages?|refugees?|tribunal|war\s+crimes|looted|testify|testimony|testifying|airstrikes?|kills?|killed|killing|strikes?|struck|attacks?|attacked|attacking|bombs?|bombed|bombing|bombings|massacres?|casualt(?:y|ies)|militants?|dead|d(?:ied|ies|ying)|wounded|evacuat(?:ed|ing|ion))\b/i;
var STRONG_URL_PATHNAME_SEGMENTS = [
  "/lifestyle/",
  "/lifestyles/",
  "/feature/",
  "/features/",
...
var STRONG_HEADLINE_PREFIX_RE2 = /^(?:photos?|gallery|in pictures)\s*:/i;
var CORROBORATING_TOKENS_RE = /(?<reunite_group>\breunit(?:e[ds]?|ing|ers?)\b|\breunions?\b)|(?<vintage>\bvintage\b)|(?<nostalgia>\bnostalgia\b)|(?<memories_group>\bmemor(?:y|ies|ial)\b)|(?<tribute_group>\btributes?\b)|(?<heartwarming>\bheartwarming\b)|(?<inspirational>\binspirational\b)|(?<feel_good>\bfeel[-\s]good\b)|(?<local_hero>\blocal\s+hero\b)|(?<unsung>\bunsung\b)|(?<decades_later>\bdecades\s+later\b)|(?<years_later>\byears\s+later\b)/gi;
var CORROBORATING_FRAMING_RE = /(?<evoking_memories>\bevoking\s+(?:powerful\s+)?memories\b)|(?<powerful_connections>\bpowerful\s+connections\b)|(?<feel_good_story>\bfeel[-\s]good\s+story\b)|(?<human_interest>\bhuman\s+interest\b)|(?<lifestyle_feature>\blifestyle\s+feature\b)|(?<gathered_to_remember>\bgathered\s+to\s+remember\b)/gi;
var CORROBORATING_PATHNAME_RE = /\/(?<travel_pathname>travel)\/|\/(?<style_pathname>style)\/|\/(?<local_pathname>local)\/|\/(?<photos_pathname>photos?)\//gi;
...
function collectGroups(text, re, groupSet) {
  ...
  for (const match of text.matchAll(re)) {
    ...
    for (const name of Object.keys(groups)) {
      if (groups[name] !== void 0) groupSet.add(name);
    }
  }
}
function classifyFeelGood(story) {
  const title = typeof story?.title === "string" ? story.title : "";
  const link = typeof story?.link === "string" ? story.link : "";
  const description = typeof story?.description === "string" ? story.description : "";
  ...
  if (STRONG_HEADLINE_PREFIX_RE2.test(title.trim())) return true;
...
// shared/ephemeral-live-classifier.js
var WATCH_LIVE_PREFIX_RE = /^\s*(?:watch\s+live|watch\s*:\s*live)\b[\s:;\-–—]*/i;
var WATCH_PREFIX_WITH_LIVE_RE = /^\s*watch\s*:\s*.*\blive\b/i;
var LIVE_PREFIX_RE = /^\s*live\s*:\s*/i;
var LIVE_EVENT_NOUN_RE = /\b(?:briefing|press\s+briefing|press\s+conference|conference|hearing|testimony|remarks|speech|address|town\s+hall|livestream|live\s+stream)\b/i;
var LIVE_PROGRAMMING_VERB_RE = /\b(?:watch|broadcast|airs?|airing|listen|tune\s+in|follow\s+live)\b/i;
function classifyEphemeralLiveCoverage(story) {
...
var CASHTAG_RE = /(?<![A-Za-z0-9$])\$([A-Z]{1,5})(?![A-Za-z0-9])/g;
function escapeRegExp(s) {
  return s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}
function buildTickerDictionary(symbols) {
  const entries = [];
  for (const entry of symbols ?? []) {
    const symbol = entry?.symbol;
    const name = entry?.name;
    if (!symbol || !name || symbol.startsWith("^")) continue;
...
var CLASSIFY_CACHE_PREFIX = "classify:sebuf:v6:";
async function buildClassifyCacheKey(title) {
  return `${CLASSIFY_CACHE_PREFIX}${(await sha256Hex4(title.toLowerCase())).slice(0, 16)}`;
}
...
var SOURCE_TIERS = source_tiers_default;
...
var STORY_TRACK_KEY = (titleHash) => `story:track:v1:${titleHash}`;
var STORY_SOURCES_KEY = (titleHash) => `story:sources:v1:${titleHash}`;
var STORY_PEAK_KEY = (titleHash) => `story:peak:v1:${titleHash}`;
var STORY_ALIAS_KEY = (titleHash) => `story:alias:v1:${titleHash}`;
var DIGEST_ACCUMULATOR_KEY = (variant, lang = "en") => `digest:accumulator:v1:${variant}:${lang}`;
var STORY_TTL = 604800;
...
// shared/diplomacy-keywords.json
var diplomacy_keywords_default = {
  diplomacyKeywords: [
    "ceasefire",
    "truce",
    "armistice",
...
var VERCEL_INITIAL_RESPONSE_LIMIT_MS = 25e3;
var DIGEST_RESPONSE_TIMEOUT_MS = 14e3;
var POST_FETCH_HEADROOM_MS = 15e3;
var OVERALL_DEADLINE_MS = VERCEL_INITIAL_RESPONSE_LIMIT_MS - POST_FETCH_HEADROOM_MS;
...
var RANK_TO_LEVEL = ["info", "low", "medium", "high", "critical"];
function capLlmUpgrade(keywordLevel, llmLevel) {
  const keywordRank = LEVEL_RANK[keywordLevel];
  const rawLlmRank = LEVEL_RANK[llmLevel];
...
var DIPLOMACY_KEYWORDS = diplomacy_keywords_default.diplomacyKeywords;
var FLASHPOINT_SCORING_KEYWORDS = diplomacy_keywords_default.flashpointKeywords;
var DIPLOMACY_FLASHPOINT_PAIRS = diplomacy_keywords_default.diplomacyFlashpointPairs;
var TICKER_DICTIONARY = buildTickerDictionary(stocks_default.symbols);
var DIPLOMACY_FLASHPOINT_BOOST = 18;
var ENTITY_CORROBORATION_SCORE_PER_SOURCE = 4;
var ENTITY_CORROBORATION_WINDOW_MS = 24 * 60 * 60 * 1e3;
var DIPLOMACY_SEVERITY_PROMOTION_MIN_TIER12_SOURCES = 3;
var MAX_DESCRIPTION_LEN = 400;
var MIN_DESCRIPTION_LEN = 40;
var DESCRIPTION_TAG_PRIORITY = {
...
function hasDiplomacyFlashpointSignal(title) {
  ...
  if (DIPLOMACY_FLASHPOINT_PAIRS.some(
    ([entity, action]) => containsKeywordToken(text, entity) && containsKeywordToken(text, action)
  )) {
    return true;
  }
  return hasAnySignal(text, DIPLOMACY_KEYWORDS) && hasAnySignal(text, FLASHPOINT_SCORING_KEYWORDS);
}
function promoteDiplomacySeverity(level, title, tier12SourceCount) {
  ...
  if (finite >= DIPLOMACY_SEVERITY_PROMOTION_MIN_TIER12_SOURCES && hasDiplomacyFlashpointSignal(title)) {
    return "high";
  }
  return level;
}
...
function computeImportanceScore(level, source, corroborationCount, publishedAt, context = {}) {
  ...
  return Math.round(
    base + diplomacyFlashpointBoost(context.title) + entityCorroborationScore(context.entityCorroborationCount)
  );
}
...
var CACHE_TTL_HEALTHY_S = 3600;
var CACHE_TTL_EMPTY_S = 300;
async function fetchAndParseRss(feed, variant, signal) {
  ...
  try {
    ...
    let source = text ? "direct" : "both-failed";
    let relayStatus = null;
    let relayBodyShape = "no-relay";
    if (!text) {
    ...
    const result = parsed ?? { items: [], parsedTotal: 0, droppedUndated: 0 };
...
function parseRssXml(xml, feed, variant) {
  ...
  if (parsedTotal > 0 && items.length === 0 && droppedUndated > 0) {
    console.warn(
      `[digest] FEED_HEALTH_WARNING all-undated feed="${feed.name}" variant=${variant} parsed=${parsedTotal} dropped=${droppedUndated}`
    );
  } else if (droppedUndated > 0) {
    console.warn(
      `[digest] partial-undated feed="${feed.name}" variant=${variant} parsed=${parsedTotal} dropped=${droppedUndated} kept=${items.length}`
    );
  }
  if (parsedTotal === 0) return null;
  return { items, parsedTotal, droppedUndated, droppedFeedCap };
}
var DESCRIPTION_TAG_REGEX_CACHE = /* @__PURE__ */ new Map();
function extractRawTagBody(xml, tag2) {
  let cached = DESCRIPTION_TAG_REGEX_CACHE.get(tag2);
  if (!cached) {
    cached = {
      cdata: new RegExp(
        `<${tag2}[^>]*>\\s*<!\\[CDATA\\[([\\s\\S]*?)\\]\\]>\\s*<\\/${tag2}>`,
        "i"
      ),
      plain: new RegExp(`<${tag2}[^>]*>([\\s\\S]*?)<\\/${tag2}>`, "i")
    };
    DESCRIPTION_TAG_REGEX_CACHE.set(tag2, cached);
  }
...
function extractDescription(block, isAtom, title) {
  const tags = isAtom ? DESCRIPTION_TAG_PRIORITY.atom : DESCRIPTION_TAG_PRIORITY.rss;
  let best = "";
  for (const tag2 of tags) {
    const raw = extractRawTagBody(block, tag2);
    if (!raw) continue;
...
for (const tag2 of KNOWN_TAGS) {
  TAG_REGEX_CACHE.set(tag2, {
    cdata: new RegExp(`<${tag2}[^>]*>\\s*<!\\[CDATA\\[([\\s\\S]*?)\\]\\]>\\s*<\\/${tag2}>`, "i"),
    plain: new RegExp(`<${tag2}[^>]*>([^<]*)<\\/${tag2}>`, "i")
  });
}
function extractTag(xml, tag2) {
  const cached = TAG_REGEX_CACHE.get(tag2);
  const cdataRe = cached?.cdata ?? new RegExp(`<${tag2}[^>]*>\\s*<!\\[CDATA\\[([\\s\\S]*?)\\]\\]>\\s*<\\/${tag2}>`, "i");
  const plainRe = cached?.plain ?? new RegExp(`<${tag2}[^>]*>([^<]*)<\\/${tag2}>`, "i");
...
function decodeXmlEntities(s) {
  return s.replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&quot;/g, '"').replace(/&apos;/g, "'").replace(/&#(\d+);/g, (_, n) => decodeNumericReference(Number(n))).replace(/&#x([0-9a-fA-F]+);/g, (_, n) => decodeNumericReference(parseInt(n, 16))).replace(/&amp;/g, "&");
}
function parseClassifyCacheHit(raw) {
  if (raw == null || typeof raw !== "object" || Array.isArray(raw)) return null;
  const { level, category } = raw;
  ...
  for (const [key, relatedItems] of keyMap) {
    ...
    for (const item of relatedItems) {
      if (hasHistoricalMarker(item.title)) {
        console.warn(
          `[classify] LLM hit forced to info by historical marker: keyword=${item.level} llm=${hit.level} title="${item.title.slice(0, 60)}"`
        );
        item.level = "info";
      ...
      if (cappedLevel !== hit.level) {
        console.warn(
          `[classify] LLM upgrade capped: keyword=${item.level} llm=${hit.level} applied=${cappedLevel} title="${item.title.slice(0, 60)}"`
        );
      }
...
function normalizeTitle(title) {
  return title.toLowerCase().replace(/\s*[-\u2013\u2014]\s*[\w\s.]+\.(?:com|org|net|co\.uk)\s*$/, "").replace(/\s*[-\u2013\u2014]\s*(?:reuters|ap news|bbc|cnn|al jazeera|france 24|dw news|pbs newshour|cbs news|nbc|abc|associated press|the guardian|nos nieuws|tagesschau|cnbc|the national)\s*$/, "").replace(/[^\p{L}\p{N}\s]/gu, "").replace(/\s+/g, " ").trim().slice(0, 120);
}
function entityKeysForTitle(title) {
  const text = normalizeScoringText(title);
  const keys = [];
  ...
  if (keys.length === 0 && hasAnySignal(text, DIPLOMACY_KEYWORDS) && hasAnySignal(text, FLASHPOINT_SCORING_KEYWORDS)) {
    keys.push("generic:diplomacy-flashpoint");
  }
  return keys;
}
function computeEntityCorroborationSignals(items, nowMs = Date.now()) {
  ...
  for (const item of items) {
    ...
    for (const key of entityKeysForTitle(item.title)) {
      ...
      if (!bucket) {
        bucket = { items: [], sources: /* @__PURE__ */ new Set(), tier12Sources: /* @__PURE__ */ new Set() };
...
function buildStoryTrackHsetFields(item, nowStr, score) {
  return [
    "lastSeen",
    nowStr,
    "currentScore",
    ...
    "publishedAt",
    Number.isFinite(item.publishedAt) ? String(item.publishedAt) : "",
    ...
    Number.isFinite(item.entityCorroborationCount) ? String(item.entityCorroborationCount) : "0",
    ...
    item.isEphemeralLiveCoverage ? "1" : "0",
...
async function writeStoryTracking(items, variant, lang, hashes, memberHashesByFinal) {
  if (items.length === 0) return;
  const now = Date.now();
  const accKey = DIGEST_ACCUMULATOR_KEY(variant, lang);
  const representativeByHash = /* @__PURE__ */ new Map();
  for (let i = 0; i < items.length; i++) {
    const hash = hashes[i];
    const item = items[i];
    const current = representativeByHash.get(hash);
    if (!current || item.importanceScore > current.importanceScore || item.importanceScore === current.importanceScore && item.publishedAt > current.publishedAt || item.importanceScore === current.importanceScore && item.publishedAt === current.publishedAt && item.title < current.title) {
      representativeByHash.set(hash, item);
    }
  }
  const writtenHashes = /* @__PURE__ */ new Set();
...
function buildDigestFeedBatches(variant, lang) {
...
async function buildDigest(variant, lang) {
  ...
  try {
    ...
    if (droppedStaleTotal > 0) {
      console.warn(
        `[digest] freshness floor dropped ${droppedStaleTotal} stale items (max age: ${maxAgeMs / (60 * 60 * 1e3)}h)`
      );
    }
    const allItems = [...results.values()].flat();
    const identityByItem = await assignStoryIdentity(allItems, normalizeTitle, sha256Hex4);
    const allMemberHashes = /* @__PURE__ */ new Set();
    for (const identity of identityByItem.values()) {
      for (const h of identity.memberTitleHashes ?? []) allMemberHashes.add(h);
    }
    const aliasTargetByHash = /* @__PURE__ */ new Map();
    ...
    await Promise.all(allItems.map(async (item) => {
      ...
      if (identity) {
        ...
        item.titleHash = await sha256Hex4(normalizeTitle(item.title));
        item.corroborationCount = 1;
      }
    }));
    const memberHashesByFinal = /* @__PURE__ */ new Map();
    for (const item of allItems) {
      ...
      if (!set) {
        ...
        memberHashesByFinal.set(item.titleHash, set);
      }
    ...
    if (diplomacySignalCount > 0 || entityCorroborationHitCount > 0) {
      console.log(
        `[digest] importance signals llm=${llmScoredCount} keywordFallback=${keywordFallbackScoredCount} diplomacy=${diplomacySignalCount} entityCorroboration=${entityCorroborationHitCount} diplomacySeverityPromotions=${diplomacySeverityPromotionCount}`
      );
    }
    ...
    await writeStoryTracking(allSliced, variant, lang, titleHashes, memberHashesByFinal).catch(
    ...
    if (VARIANT_FEEDS[variant] && /^[a-z]{2}$/.test(lang)) {
      await setCachedJson(`news:coverage-ledger:v1:${variant}:${lang}`, ledger, 7200).catch(
        (err) => console.warn("[digest] coverage-ledger write failed:", err)
      );
    }
    return {
      categories,
