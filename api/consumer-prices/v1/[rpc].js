...
function createConsumerPricesServiceRoutes(handler, options) {
  return [
    {
      method: "GET",
...
async function getConsumerPriceOverview(_ctx, req) {
...
init_redis();
var DEFAULT_MARKET2 = "ae";
var DEFAULT_BASKET = "essentials-ae";
var DEFAULT_RANGE = "30d";
var VALID_RANGES = /* @__PURE__ */ new Set(["7d", "30d", "90d", "180d"]);
async function getConsumerPriceBasketSeries(_ctx, req) {
  const market = req.marketCode || DEFAULT_MARKET2;
...
init_redis();
var DEFAULT_MARKET3 = "ae";
var DEFAULT_RANGE2 = "30d";
var VALID_RANGES2 = /* @__PURE__ */ new Set(["7d", "30d", "90d", "180d"]);
async function listConsumerPriceCategories(_ctx, req) {
  const market = req.marketCode || DEFAULT_MARKET3;
  const range = VALID_RANGES2.has(req.range ?? "") ? req.range : DEFAULT_RANGE2;
...
init_redis();
var DEFAULT_MARKET4 = "ae";
var DEFAULT_RANGE3 = "30d";
var VALID_RANGES3 = /* @__PURE__ */ new Set(["7d", "30d", "90d"]);
async function listConsumerPriceMovers(_ctx, req) {
  const market = req.marketCode || DEFAULT_MARKET4;
  const range = VALID_RANGES3.has(req.range ?? "") ? req.range : DEFAULT_RANGE3;
  ...
  try {
    const cached = await getCachedJson(key, true);
    if (!cached) return EMPTY2;
    const limit = req.limit ?? 10;
    const filterCategory = req.categorySlug;
    const filter = (movers) => (filterCategory ? movers.filter((m) => m.category === filterCategory) : movers).slice(0, limit);
...
init_redis();
var DEFAULT_MARKET5 = "ae";
var DEFAULT_BASKET2 = "essentials-ae";
async function listRetailerPriceSpreads(_ctx, req) {
  const market = req.marketCode || DEFAULT_MARKET5;
  const basket = req.basketSlug || DEFAULT_BASKET2;
...
init_redis();
var DEFAULT_MARKET6 = "ae";
async function getConsumerPriceFreshness(_ctx, req) {
  const market = req.marketCode || DEFAULT_MARKET6;
