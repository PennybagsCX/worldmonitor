...
var CLIMATE_ANOMALIES_KEY = "climate:anomalies:v2";
var CLIMATE_AIR_QUALITY_KEY = "climate:air-quality:v1";
var CLIMATE_CO2_MONITORING_KEY = "climate:co2-monitoring:v1";
var CLIMATE_OCEAN_ICE_KEY = "climate:ocean-ice:v1";
var CLIMATE_NEWS_KEY = "climate:news-intelligence:v1";
...
init_redis();
var getCo2Monitoring = async (ctx, _req) => {
  try {
    const cached = await getCachedJson(CLIMATE_CO2_MONITORING_KEY, true);
    return cached ?? markNoStoreFallbackResponse(ctx.request, {});
  } catch {
...
function pickKey(record, snakeKey, camelKey) {
...
function normalizeOceanIceSeed(value) {
  ...
  const onlyEmptyTrend = normalizedKeys.length === 1 && Array.isArray(normalized.iceTrend12m) && normalized.iceTrend12m.length === 0;
  if (normalizedKeys.length === 0 || onlyEmptyTrend) {
    return {};
  }
  return { data: normalized };
}
var getOceanIceData = async (ctx, _req) => {
  try {
...
function pickKey2(record, snakeKey, camelKey) {
  if (record[snakeKey] != null) return record[snakeKey];
  return record[camelKey];
}
function normalizeAirQualityStation(value) {
  const record = asRecord2(value);
  if (!record) return null;
  const city = asString2(record.city);
  const lat = asNumber(record.lat);
  const lng = asNumber(record.lng);
  const pm25 = asNumber(record.pm25);
  const aqi = asNumber(record.aqi);
  const measuredAt = asNumber(pickKey2(record, "measured_at", "measuredAt"));
  if (!city || lat == null || lng == null || pm25 == null || aqi == null || measuredAt == null) {
    return null;
  }
  return {
    city,
    countryCode: asString2(pickKey2(record, "country_code", "countryCode")),
    lat,
    lng,
    pm25,
    aqi: Math.max(0, Math.min(500, Math.round(aqi))),
    riskLevel: asString2(pickKey2(record, "risk_level", "riskLevel")),
    pollutant: asString2(record.pollutant) || "pm25",
    measuredAt: Math.round(measuredAt),
    source: asString2(record.source)
  };
}
function normalizeAirQualityStations(value) {
  if (!Array.isArray(value)) return [];
  return value.map((entry) => normalizeAirQualityStation(entry)).filter((entry) => entry != null);
}
function normalizeAirQualityFetchedAt(value) {
  const record = asRecord2(value);
  if (!record) return 0;
  const numeric = asNumber(pickKey2(record, "fetched_at", "fetchedAt"));
  return numeric == null ? 0 : Math.round(numeric);
}

// server/worldmonitor/climate/v1/list-air-quality-data.ts
init_redis();
var listAirQualityData = async (_ctx, _req) => {
  const payload = await getCachedJson(CLIMATE_AIR_QUALITY_KEY, true);
...
init_redis();
var listClimateAnomalies = async (ctx, _req) => {
  try {
    const result = await getCachedJson(CLIMATE_ANOMALIES_KEY, true);
    if (!result?.anomalies) {
      return markNoStoreFallbackResponse(ctx.request, { anomalies: [], pagination: void 0 });
    }
...
init_redis();
var SEED_CACHE_KEY = "climate:disasters:v1";
...
function normalizeCachedDisaster(row) {
  ...
  return {
    ...
    lat: asNumber2(record.lat, 0),
    lng: asNumber2(record.lng, 0),
    severity: String(record.severity || ""),
    startedAt: asNumber2(record.startedAt ?? record.started_at, 0),
    status: String(record.status || ""),
    affectedPopulation: asNumber2(record.affectedPopulation ?? record.affected_population, 0),
    source: String(record.source || ""),
    sourceUrl: String(record.sourceUrl || record.source_url || "")
  };
}
var listClimateDisasters = async (ctx, req) => {
  try {
    ...
    const allDisasters = result.disasters.map(normalizeCachedDisaster).filter((row) => row != null);
...
init_redis();
var listClimateNews = async (_ctx, _req) => {
  try {
    const result = await getCachedJson(CLIMATE_NEWS_KEY, true);
