...
var RATE_LIMIT_SCOPE = "/api/mcp-proxy";
...
var RATE_LIMIT_ERROR_CODE = -32029;
...
var SSE_RPC_TIMEOUT_MS = process.env.NODE_TEST_CONTEXT ? 200 : 12e3;
var MCP_PROTOCOL_VERSION = "2025-03-26";
function withProxyNoStore(headers = {}) {
...
var TEST_RESOLVER_KEY = /* @__PURE__ */ Symbol.for("worldmonitor.mcpProxy.resolveHostnameForTest");
function getResolveHostnameForTest() {
  if (!process.env.NODE_TEST_CONTEXT) return null;
  const resolver = globalThis[TEST_RESOLVER_KEY];
  return typeof resolver === "function" ? resolver : null;
}
var McpProxySsrfError = class extends Error {
  constructor(message2) {
    super(message2);
    this.name = "McpProxySsrfError";
  }
};
var SSRF_BLOCKED_PUBLIC_MESSAGE = "serverUrl host is not allowed";
...
async function resolveDnsJson(hostname, recordType) {
  ...
  return (Array.isArray(data?.Answer) ? data.Answer : []).filter((answer) => answer?.type === expectedType && typeof answer?.data === "string").map((answer) => answer.data);
}
...
async function assertServerUrlSafe(url) {
  ...
  try {
    ...
    throw new McpProxySsrfError(`serverUrl DNS resolution failed: ${message2}`);
  }
...
var DENIED_FORWARD_HEADERS = /* @__PURE__ */ new Set([
  "metadata-flavor",
  "metadata",
...
function captureMeta(serverUrl, customHeaders, meta) {
  meta.targetHost = serverUrl.hostname;
  meta.targetPath = serverUrl.pathname;
  meta.headerNames = Object.keys(customHeaders || {}).filter((k) => typeof k === "string" && !k.includes("\r") && !k.includes("\n")).sort();
}
async function handleListTools(req, cors, meta) {
...
async function handleCallTool(req, cors, meta) {
...
async function handler(req) {
  ...
  const meta = { targetHost: "", targetPath: "", headerNames: [] };
