...
// src/generated/server/worldmonitor/batch/v1/service_server.ts
...
function createBatchServiceRoutes(handler, options) {
  return [
    {
      method: "POST",
...
// server/worldmonitor/batch/v1/execute-batch.ts
var MAX_BATCH_OPERATIONS = 20;
var MAX_OPERATION_ID_LENGTH = 64;
var MAX_OPERATION_PATH_LENGTH = 2048;
var SUB_REQUEST_TIMEOUT_MS = 1e4;
var MAX_SUB_RESPONSE_BYTES = 1048576;
var BATCH_MARKER_HEADER = "x-wm-batch";
var FORWARDED_HEADERS = ["authorization", "x-worldmonitor-key", "x-api-key", "accept-language"];
var RPC_PATH_RE = /^\/api\/[a-z][a-z0-9-]*\/v\d+\/[a-z][a-z0-9-]*$/;
var V2_PATH_RE = /^\/api\/v2\/[a-z][a-z0-9-]*\/[a-z][a-z0-9-]*$/;
var DEFAULT_SUB_REQUEST_USER_AGENT = "WorldMonitor-Batch/1.0 (+https://www.worldmonitor.app/openapi.json)";
function validateOperations(operations, origin) {
  const validated = [];
  const violations = [];
...
function buildSubRequestHeaders(inbound) {
...
async function runOperation(op, headers, fetchImpl) {
  if (!op.target) {
  ...
  return { id: op.id, status: response.status, body, error: "" };
}
function createExecuteBatch(fetchImpl = (input, init) => fetch(input, init)) {
  return async function executeBatch2(ctx, req) {
...
var executeBatch = createExecuteBatch();

// server/worldmonitor/batch/v1/handler.ts
var batchHandler = {
  executeBatch
};
