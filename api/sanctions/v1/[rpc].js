...
function createSanctionsServiceRoutes(handler, options) {
  return [
    {
      method: "GET",
...
// server/worldmonitor/sanctions/v1/list-sanctions-pressure.ts
var REDIS_CACHE_KEY = "sanctions:pressure:v1";
var DEFAULT_MAX_ITEMS = 25;
var MAX_ITEMS_LIMIT = 60;
...
var listSanctionsPressure = async (ctx, req) => {
  const isPro = await isCallerPremium(ctx.request);
  if (!isPro) return emptyResponse();
  const maxItems = clampMaxItems(req.maxItems);
  try {
    const data = await getCachedJson(REDIS_CACHE_KEY, true);
...
init_redis();
var ENTITY_INDEX_KEY = "sanctions:entities:v1";
var DEFAULT_MAX = 10;
var MAX_RESULTS_LIMIT = 50;
var MAX_QUERY_LENGTH = 200;
var MIN_QUERY_LENGTH = 2;
var OPENSANCTIONS_BASE = "https://api.opensanctions.org";
var OPENSANCTIONS_TIMEOUT_MS = 8e3;
var OPENSANCTIONS_CACHE_TTL_SECONDS = 3600;
...
function lookupCacheKey(q, maxResults) {
...
async function searchOpenSanctions(q, limit) {
  ...
  const results = hits.map(normalizeOpenSanctionsHit).filter((r) => r !== null);
  const total = data.total?.value ?? results.length;
  return { results, total };
}
function searchOfacLocal(q, maxResults, raw) {
  ...
  const results = scored.slice(0, maxResults).map(({ entry }) => ({
    id: entry.id,
    name: entry.name,
    entityType: entry.et,
...
var lookupSanctionEntity = async (_ctx, req) => {
  const q = (req.q ?? "").trim();
  ...
  try {
    const upstream = await cachedFetchJson(
