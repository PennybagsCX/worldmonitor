...
async function validateUserApiKey(key) {
  ...
  const cacheKey2 = `${CACHE_KEY_PREFIX}${keyHash}`;
  try {
...
function getScopedRatelimit(scope, limit, window) {
  const cacheKey2 = `${scope}|${limit}|${window}`;
  const cached = scopedLimiters.get(cacheKey2);
  if (cached) return cached;
  const url = process.env.UPSTASH_REDIS_REST_URL;
  const token = process.env.UPSTASH_REDIS_REST_TOKEN;
  if (!url || !token) return null;
  ...
  scopedLimiters.set(cacheKey2, rl);
  return rl;
}
...
function createImageryServiceRoutes(handler, options) {
  return [
    {
      method: "GET",
...
// server/worldmonitor/imagery/v1/search-imagery.ts
var STAC_SEARCH = "https://earth-search.aws.element84.com/v1/search";
var COLLECTIONS = ["sentinel-2-l2a", "sentinel-1-grd"];
var CACHE_TTL = 3600;
function fnv1a(str) {
  let hash = 2166136261;
...
function cacheKey(bbox, datetime, source, limit) {
...
function mapFeature(f) {
  ...
  const thumbnail = s3ToHttps(
    f.assets?.thumbnail?.href ?? f.assets?.overview?.href ?? f.links?.find((l) => l.rel === "thumbnail")?.href ?? ""
  );
...
async function searchImagery(_ctx, req) {
  if (!req.bbox) {
  ...
  const nowHour = /* @__PURE__ */ new Date();
  nowHour.setMinutes(0, 0, 0);
  const weekAgo = new Date(nowHour.getTime() - 7 * 24 * 60 * 60 * 1e3);
