...
// server/_shared/pro-entitlement.ts
async function checkProEntitlement(userId, clerkRole, corsHeaders, loadEntitlements = getEntitlements) {
  if (clerkRole === "pro") return { allowed: true };
  return checkTierProEntitlement(userId, corsHeaders, loadEntitlements);
}
async function checkTierProEntitlement(userId, corsHeaders, loadEntitlements = getEntitlements) {
...
var BRIEF_TTL_SECONDS = 7 * 24 * 60 * 60;
...
async function handler(req, ctx) {
  ...
  const callerProvidedSlot = typeof issueSlot === "string" && issueSlot.trim().length > 0;
  if (!callerProvidedSlot) {
    try {
      const latest = await readRawJsonFromUpstash(`brief:latest:${session.userId}`);
      const slot = latest?.issueSlot;
      ...
      console.error("[api/brief/share-url] latest pointer read failed:", err.message);
  ...
  try {
    ...
    console.error("[api/brief/share-url] Upstash read failed:", err.message);
