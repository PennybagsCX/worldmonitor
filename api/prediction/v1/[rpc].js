...
var TECH_CATEGORY_TAGS = openapi_filter_param_contracts_default.predictionMarketTechCategories;
var FINANCE_CATEGORY_TAGS = openapi_filter_param_contracts_default.predictionMarketFinanceCategories;
var DEGENERATE_MARKET_SLUGS = /* @__PURE__ */ new Set(["undefined", "null", "nan", ""]);
function bootstrapMarketIdentity(market) {
...
function dedupeBootstrapMarkets(markets) {
...
function toProtoMarket(m, category) {
  return {
    id: m.url?.split("/").pop() || "",
    title: m.title,
...
var listPredictionMarkets = async (_ctx, req) => {
  try {
    ...
    if (!bootstrap) return { markets: [], pagination: void 0, fetchedAt: 0, dataAvailable: false };
    ...
    const variant = isTech ? bootstrap.tech : isFinance ? bootstrap.finance : category ? bootstrap.geopolitical : unionAllPools();
    if (!variant || variant.length === 0) return { markets: [], pagination: void 0, fetchedAt, dataAvailable: false };
    let markets = variant.map((m) => toProtoMarket(m, category));
    if (query) {
      const q = query.toLowerCase();
      markets = markets.filter((m) => m.title.toLowerCase().includes(q));
    }
    return { markets: markets.slice(0, limit), pagination: void 0, fetchedAt, dataAvailable: true };
  } catch {
    return { markets: [], pagination: void 0, fetchedAt: 0, dataAvailable: false };
  }
};
