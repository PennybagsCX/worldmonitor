...
init_redis();
var SEED_KEY_PREFIX = "research:arxiv:v1";
async function listArxivPapers(ctx, req) {
  try {
    ...
    if (!result?.papers?.length) return markNoStoreFallbackResponse(ctx.request, { papers: [], pagination: void 0 });
    return { papers: result.papers.slice(0, pageSize), pagination: void 0 };
  } catch {
    return markNoStoreFallbackResponse(ctx.request, { papers: [], pagination: void 0 });
  }
}

// server/worldmonitor/research/v1/list-trending-repos.ts
init_redis();
var SEED_KEY_PREFIX2 = "research:trending:v1";
async function listTrendingRepos(ctx, req) {
  try {
    const language = req.language || "python";
    const period = req.period || "daily";
    const pageSize = clampInt(req.pageSize, 50, 1, 100);
    const seedKey = `${SEED_KEY_PREFIX2}:${language}:${period}:50`;
    const result = await getCachedJson(seedKey, true);
    if (!result?.repos?.length) return markNoStoreFallbackResponse(ctx.request, { repos: [], pagination: void 0 });
    return { repos: result.repos.slice(0, pageSize), pagination: void 0 };
  } catch {
    return markNoStoreFallbackResponse(ctx.request, { repos: [], pagination: void 0 });
  }
}
...
init_redis();
var SEED_KEY_PREFIX3 = "research:hackernews:v1";
var ALLOWED_HN_FEEDS = new Set(openapi_filter_param_contracts_default.researchHackerNewsFeedTypes);
async function listHackernewsItems(ctx, req) {
  try {
    const feedType = ALLOWED_HN_FEEDS.has(req.feedType) ? req.feedType : "top";
    const pageSize = clampInt(req.pageSize, 30, 1, 100);
    const seedKey = `${SEED_KEY_PREFIX3}:${feedType}:30`;
    const result = await getCachedJson(seedKey, true);
    if (!result?.items?.length) return markNoStoreFallbackResponse(ctx.request, { items: [], pagination: void 0 });
    return { items: result.items.slice(0, pageSize), pagination: void 0 };
  } catch {
    return markNoStoreFallbackResponse(ctx.request, { items: [], pagination: void 0 });
  }
}
...
// server/worldmonitor/research/v1/_tech-events-paging.ts
function valueOrDefault(value, fallback, isPresent) {
  if (isPresent === false || value === void 0) return fallback;
  return value;
}
function resolveTechEventsPaging(req, presence = {}) {
  return {
...
// server/worldmonitor/research/v1/list-tech-events.ts
var REDIS_CACHE_KEY = "research:tech-events:v1";
var REDIS_CACHE_TTL = 21600;
var TECH_EVENTS_UNAVAILABLE_ERROR = "tech events unavailable: no upstream data";
var ICS_URL = "https://www.techmeme.com/newsy_events.ics";
var DEV_EVENTS_RSS = "https://dev.events/rss.xml";
var FETCH_TIMEOUT_MS = 8e3;
var TECH_EVENT_TYPES = new Set(openapi_filter_param_contracts_default.researchTechEventTypes);
function readTechEventsPagingPresence(ctx) {
...
var RSS_ACCEPT = "application/rss+xml, application/xml, text/xml, text/calendar, */*";
async function fetchTextWithRelay(url) {
  try {
    ...
    console.warn(`[tech-events] Direct fetch ${url} failed: ${e.message}`);
  }
  ...
  if (relayBase) {
    try {
      ...
      console.warn(`[tech-events] Relay fetch ${url} failed: ${e.message}`);
    }
  }
  return null;
}
...
function normalizeLocation(location2) {
  ...
  let normalized = location2.toLowerCase().trim();
  ...
  if (CITY_COORDS[normalized]) {
    ...
    return { lat: c.lat, lng: c.lng, country: c.country, original: location2, virtual: c.virtual ?? false };
  }
  ...
  for (const [key, coords] of Object.entries(CITY_COORDS)) {
    if (normalized.includes(key) || key.includes(normalized)) {
      return { lat: coords.lat, lng: coords.lng, country: coords.country, original: location2, virtual: coords.virtual ?? false };
    }
  }
  return null;
}
function parseICS(icsText) {
  ...
  for (const block of eventBlocks) {
    ...
    if (summaryMatch && dtstartMatch) {
      ...
      const location2 = locationMatch ? locationMatch[1].trim() : "";
      ...
      else if (location2) type = "conference";
      const coords = normalizeLocation(location2 || null);
      events.push({
        id: uid,
        title: summary,
        type,
...
function parseDevEventsRSS(rssText) {
  ...
  for (const match of itemMatches) {
    ...
    if (locationMatch) {
      location2 = locationMatch[1].trim();
    }
    ...
    const coords = location2 && location2 !== "Online" ? normalizeLocation(location2) : null;
    events.push({
      id: guid || `dev-events-${title.slice(0, 20)}`,
      title,
      type: "conference",
      location: location2 || "",
      coords: coords ?? (location2 === "Online" ? { lat: 0, lng: 0, country: "Virtual", original: "Online", virtual: true } : void 0),
      startDate,
      endDate: startDate,
...
async function fetchAllTechEvents() {
  ...
  events = events.filter((e) => {
    const year2 = e.startDate.slice(0, 4);
    const key = e.title.toLowerCase().replace(/[^a-z0-9]/g, "").slice(0, 30) + year2;
...
function filterEvents(events, req, pagingPresence) {
...
async function fetchWidestTechEvents() {
...
async function listTechEvents(ctx, req) {
  try {
    ...
    if (!result || result.events.length === 0) {
      return { success: true, count: 0, conferenceCount: 0, mappableCount: 0, lastUpdated: (/* @__PURE__ */ new Date()).toISOString(), events: [], error: TECH_EVENTS_UNAVAILABLE_ERROR };
    }
