...
it('binds each admission to its URL, method and credentials', async () => {
  ...
  for (const changed of [request(url + 'X'), request(url, 'other'), request(url, 'test-key', 'POST')]) {
    assert.equal(await consumeSubRequestAdmission(await marked(request(), changed)), false);
  }
  assert.equal(await consumeSubRequestAdmission(await marked()), true);
});
...
it('expires admissions and keeps credentials out of Redis', async () => {
  ...
  now += 31_000;
  assert.equal(await consumeSubRequestAdmission(outbound), false);
});
it('does not grant admission when Redis is unavailable', async () => {
  ...
  assert.equal(await consumeSubRequestAdmission(new Request(request(), { headers })), false);
});

it('accepts router echoes and query ordering without ignoring caller data', async () => {
  installRedis({});
  assert.equal(await consumeSubRequestAdmission(await marked(request(), request(url + '&rpc=list-market-quotes'))), true);
  assert.equal(await consumeSubRequestAdmission(await marked(request(), request(url + '&rpc=other'))), false);
  assert.equal(await consumeSubRequestAdmission(await marked(request(url + '#not-sent'), request())), true);
  const reordered = url.replace('?symbols=AAPL', '?lang=en&symbols=AAPL');
  assert.equal(await consumeSubRequestAdmission(await marked(request(url + '&lang=en'), request(reordered))), true);
});
...
it('never waives a principal bucket using an IP or different-scope admission', async () => {
  ...
  for (const charged of [null, 'api_key:user1', 'session:user2']) {
    ...
    assert.equal(await consumeSubRequestAdmission(outbound, 'session:user1'), false);
  }
  const outbound = request();
  const token = await issueSubRequestAdmission(outbound, 'session:user1');
  assert.ok(token);
  outbound.headers.set(SUB_REQUEST_MARKER_HEADER, token);
  assert.equal(await consumeSubRequestAdmission(outbound, 'session:user1'), true);
});
