...
async function fetchUnhcrYearItems(year2) {
  const limit = 1e4;
  const maxPageGuard = 25;
  const items = [];
  for (let page = 1; page <= maxPageGuard; page++) {
    const response = await fetch(
      `https://api.unhcr.org/population/v1/population/?year=${year2}&limit=${limit}&page=${page}&coo_all=true&coa_all=true`,
...
async function trySeededData(req) {
  try {
    const year2 = req.year > 0 ? req.year : (/* @__PURE__ */ new Date()).getFullYear();
    const seedKey = `${REDIS_CACHE_KEY}:${year2}`;
...
async function getDisplacementSummary(_ctx, req) {
  ...
  try {
    ...
    const year2 = req.year > 0 ? req.year : (/* @__PURE__ */ new Date()).getFullYear();
    const cacheKey = `${REDIS_CACHE_KEY}:${year2}`;
