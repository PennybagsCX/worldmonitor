      ...
      handler: async (req) => {
        try {
          ...
          const body = {
            ...
            direction: params.get("direction") ?? "FLIGHT_DIRECTION_UNSPECIFIED",
      ...
      handler: async (req) => {
        try {
          ...
          const body = {
            ...
            cabin: params.get("cabin") ?? "CABIN_CLASS_UNSPECIFIED",
...
// server/worldmonitor/aviation/v1/_counters.ts
var counters = {
...
function incrementProviderCounter(provider, amount = 1) {
...
var NOTAM_RESTRICTION_QCODES = /* @__PURE__ */ new Set(["RA", "RO"]);
...
async function fetchNotamClosures(airports) {
  ...
  const result = { closedIcaoCodes: /* @__PURE__ */ new Set(), restrictedIcaoCodes: /* @__PURE__ */ new Set(), notamsByIcao: /* @__PURE__ */ new Map() };
  if (!apiKey) {
...
var NOTAM_CACHE_KEY = "aviation:notam:closures:v2";
var NOTAM_CACHE_TTL = 1800;
var SEED_FRESHNESS_MS = 20 * 60 * 1e3;
...
var FAA_AIRPORT_SET = new Set(FAA_AIRPORTS);
var INTL_AIRPORT_SET = new Set(AVIATIONSTACK_AIRPORTS);
async function listAirportDelays(_ctx, _req) {
  ...
  let intlAlerts = [];
  let intlCoverage = [];
  let intlCoveredIatas = /* @__PURE__ */ new Set();
  try {
    ...
    if (cached && Array.isArray(cached.alerts)) {
      ...
      if (Array.isArray(cached.coverage)) {
        ...
        intlCoveredIatas = new Set(cached.coverage.filter((hub) => hub.status === "normal" || hub.status === "disruption").map((hub) => hub.iata));
      }
    }
  } catch (err) {
...
init_redis();
var SEED_CACHE_KEY = "aviation:delays:intl:v3";
async function getAirportOpsSummary(_ctx, req) {
  ...
  try {
    ...
    let notamRestrictedIcaos = /* @__PURE__ */ new Set();
    let notamReasons = {};
    try {
      const notamResult = await loadNotamClosures();
      if (notamResult) {
...
init_redis();
function aviationStackBudgetMonth(now = /* @__PURE__ */ new Date()) {
  const ym = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}`;
  return ym;
}
function avstackBudgetKey(now = /* @__PURE__ */ new Date()) {
  return `aviation:avstack:calls:${aviationStackBudgetMonth(now)}`;
}
var AVSTACK_BUDGET_TTL = 40 * 24 * 60 * 60;
...
async function reserveAviationStackCalls(count, kind) {
  if (count <= 0) return true;
  const hardCap = intEnv("AVIATIONSTACK_MONTHLY_BUDGET", 13e4);
  if (hardCap <= 0) return true;
  const ceiling = kind === "seed" ? hardCap : Math.min(hardCap, intEnv("AVIATIONSTACK_REQUEST_BUDGET", 85e3));
...
async function listAirportFlights(ctx, req) {
  ...
  try {
    const result = await cachedFetchJson(
      ...
      async () => {
        ...
        const childUnavailableSources = /* @__PURE__ */ new Set();
        ...
        if (childUnavailableSources.size > 0) {
          unavailableSource = successfulChildren > 0 ? "partial" : [...childUnavailableSources][0] ?? "unavailable";
          return null;
        }
...
async function getFlightStatus(ctx, req) {
  const flightNumber = (req.flightNumber?.toUpperCase().replace(/\s/g, "") || "").replace(/^([A-Z]{2,3})0+(\d+)$/, "$1$2");
...
init_redis();
var CACHE_TTL4 = 120;
var CALLSIGN_CACHE_TTL = 60;
var CALLSIGN_NEGATIVE_TTL = 10;
var BBOX_RELAY_TIMEOUT_MS = 6e3;
function isDegenerateBbox(req) {
...
async function trackAircraft(_ctx, req) {
  ...
  let result = null;
  try {
    const positiveTtl = req.callsign ? CALLSIGN_CACHE_TTL : CACHE_TTL4;
    const negativeTtl = req.callsign ? CALLSIGN_NEGATIVE_TTL : CACHE_TTL4;
      ...
      async () => {
        ...
        if (!isCallsignOnly && relayBase && req.icao24) {
          try {
            ...
            const resp = await fetch(osUrl, { headers: getRelayHeaders({}), signal: AbortSignal.timeout(8e3) });
            if (resp.ok) {
              const data = await resp.json();
...
async function searchPricesTravelpayouts(opts) {
  ...
  if (isDayPrecision) {
    ...
    const data = await cachedFetchJson(
      cacheKey,
      3600,
      () => fetchTp(`${BASE_V3}/prices_for_dates?${params}`, token)
    );
    ...
    const data = await cachedFetchJson(
      cacheKey,
      7200,
      () => fetchTp(`${BASE_V2}/month-matrix?${params}`, token)
    );
    if (data === null) {
      upstreamFailed = true;
    } else {
    ...
    const data = await cachedFetchJson(
      cacheKey,
      3600,
      () => fetchTp(`${BASE_V2}/latest?${params}`, token)
    );
    if (data === null) {
      upstreamFailed = true;
    } else {
...
function emptyDegraded(now, error, provider) {
  return {
    quotes: [],
    provider,
    isDemoMode: false,
...
async function searchFlightPrices(_ctx, req) {
  ...
  if (token) {
    try {
      ...
      const error = result.upstreamFailed ? "upstream_error" : "no_results";
      if (demoOptIn) {
...
var POSITIVE_TTL = 60;
var NEGATIVE_TTL = 30;
function emptyResult(error, channelExists = false) {
  return {
    videoId: "",
    isLive: false,
    channelExists,
...
function parseChannelHtml(html) {
  ...
  if (detailsIndex !== -1) {
    const detailsBlock = html.substring(detailsIndex, detailsIndex + 5e3);
...
async function tryChannelScrape(channel2) {
  try {
    const channelHandle = channel2.startsWith("@") ? channel2 : `@${channel2}`;
...
async function fetchLiveStreamInfo(channel2, videoId, query) {
  ...
  if (channel2) {
    const scrapeResult = await tryChannelScrape(channel2);
    if (scrapeResult) return scrapeResult;
  }
  return null;
}
var getYoutubeLiveStreamInfo = async (_ctx, req) => {
  const { channel: channel2, videoId } = req;
  const params = new URLSearchParams();
  if (channel2) params.set("channel", channel2);
  if (videoId) params.set("videoId", videoId);
  if (!params.toString()) {
    return emptyResult("Missing channel or videoId");
  }
  const normalizedChannel = channel2.replace(/^@/, "");
  const cacheKey = `aviation:yt-live:vid:${videoId}:ch:${normalizedChannel}:v1`;
  const cached = await cachedFetchJson(
    cacheKey,
    POSITIVE_TTL,
    () => fetchLiveStreamInfo(channel2, videoId, params.toString()),
    NEGATIVE_TTL
  );
  if (cached) return cached;
  return emptyResult("Failed to detect live status", Boolean(channel2));
};

// server/worldmonitor/aviation/v1/search-google-flights.ts
init_redis();
var CACHE_TTL6 = 600;
async function searchGoogleFlights(_ctx, req) {
...
init_redis();
var CACHE_TTL7 = 600;
async function searchGoogleDates(_ctx, req) {
  const origin = (req.origin || "").toUpperCase().trim();
  const destination = (req.destination || "").toUpperCase().trim();
