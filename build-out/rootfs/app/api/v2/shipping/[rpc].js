...
function createShippingV2ServiceRoutes(handler, options) {
  return [
    {
      method: "GET",
...
// server/worldmonitor/shipping/v2/route-intelligence.ts
var VALID_CARGO_TYPES = /* @__PURE__ */ new Set(["container", "tanker", "bulk", "roro"]);
async function routeIntelligence(ctx, req) {
  ...
  const cargoType = VALID_CARGO_TYPES.has(cargoTypeRaw) ? cargoTypeRaw : "container";
  const hs2 = (req.hs2 ?? "").trim().replace(/\D/g, "") || "27";
  const clusters = country_port_clusters_default;
  ...
  const statusAvailable = Array.isArray(statusRaw?.chokepoints) && statusRaw.chokepoints.length > 0 && statusRaw.upstreamUnavailable !== true;
  ...
  const chokepointExposures = CHOKEPOINT_REGISTRY.filter((cp) => cp.routeIds.some((r) => relevantRouteSet.has(r))).map((cp) => {
  ...
  const bypassOptions = primaryChokepoint ? (BYPASS_CORRIDORS_BY_CHOKEPOINT[primaryChokepoint.chokepointId] ?? []).filter((c) => c.suitableCargoTypes.length === 0 || c.suitableCargoTypes.includes(cargoType)).slice(0, 5).map((c) => ({
    id: c.id,
    name: c.name,
    type: c.type,
  ...
  return {
    ...
    warRiskTier,
    disruptionScore,
    fetchedAt: statusAvailable ? (/* @__PURE__ */ new Date()).toISOString() : ""
  };
}
...
// server/_shared/ip-address-classification.ts
...
function isBlockedDottedIpv4(addr) {
...
function isBlockedEmbeddedIpv4(hi, lo) {
  return isBlockedIpv4(hi >> 8 & 255, hi & 255, lo >> 8 & 255);
}
...
function isBlockedResolvedAddress(address) {
  ...
  if (g0 === 8193 && g1 === 3512) return true;
  if (groups.slice(0, 5).every((part) => part === 0) && g5 === 65535) {
    return isBlockedEmbeddedIpv4(g6, g7);
  }
  if (g0 === 100 && g1 === 65435 && groups.slice(2, 6).every((part) => part === 0)) {
    return isBlockedEmbeddedIpv4(g6, g7);
  }
  ...
  if (g0 === 8194) {
    return isBlockedEmbeddedIpv4(g1, g2);
  }
  return false;
}
...
var TEST_RESOLVER_KEY = /* @__PURE__ */ Symbol.for("worldmonitor.shippingV2.resolveWebhookHostnameForTest");
...
function isBlockedCallbackUrl(rawUrl) {
  let parsed;
  try {
...
async function defaultResolveHostname(hostname) {
  ...
  const resolveRecordType = async (recordType) => {
...
async function assertCallbackUrlRegistrationSafe(callbackUrl, resolveHostname = defaultResolveHostname) {
  ...
  try {
    ...
    throw new Error(`callbackUrl DNS resolution failed: ${message2}`);
  }
...
// server/worldmonitor/shipping/v2/register-webhook.ts
async function registerWebhook(ctx, req) {
  const apiKeyResult = await validateApiKey(ctx.request, { forceKey: true });
  ...
  try {
    ...
    const message2 = error instanceof Error ? error.message : "callbackUrl is not allowed";
    throw new ValidationError([{ field: "callbackUrl", description: message2 }]);
  }
...
init_redis();
async function listWebhooks(ctx, _req) {
  const apiKeyResult = await validateApiKey(ctx.request, { forceKey: true });
  if (apiKeyResult.required && !apiKeyResult.valid) {
    throw new ApiError(401, apiKeyResult.error ?? "API key required", "");
  }
  ...
  const memberIds = smembersResult[0]?.result ?? [];
