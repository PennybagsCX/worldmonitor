...
function createDefaultNotificationChannelsDeps() {
  return {
    validateBearerToken,
    getEntitlements,
...
var DENIAL_CAPTURE_DEDUP_WINDOW_MS = 6e4;
var lastDenialCaptureAtByCode = /* @__PURE__ */ new Map();
function shouldCaptureDenial(code, now) {
  if (!code || code === "subscription_lapsed") return false;
  const last = lastDenialCaptureAtByCode.get(code);
  if (last !== void 0 && now - last < DENIAL_CAPTURE_DEDUP_WINDOW_MS) return false;
  lastDenialCaptureAtByCode.set(code, now);
  return true;
}
function __resetDenialCaptureDedupForTests() {
  lastDenialCaptureAtByCode.clear();
}
var notificationChannelsDeps = createDefaultNotificationChannelsDeps();
function __setNotificationChannelsDepsForTests(overrides) {
  notificationChannelsDeps = overrides ? { ...createDefaultNotificationChannelsDeps(), ...overrides } : createDefaultNotificationChannelsDeps();
}
...
var CONVEX_RELAY_TIMEOUT_MS = 15e3;
async function convexRelay(body, signal = AbortSignal.timeout(CONVEX_RELAY_TIMEOUT_MS)) {
...
async function convexRelayWithDurableWelcome(body) {
  ...
  if (capability.ok) {
    const payload2 = await capability.json().catch(() => null);
    if (payload2?.durableWelcomeScheduling !== true) {
  ...
  const payload = await capability.clone().json().catch(() => null);
...
async function handler(req, ctx) {
  ...
  if (req.method === "POST") {
    ...
    const finishDurableWelcomeRelay = async (relay, relayAction, welcomeChannelType) => {
      ...
      if (result.isNew && (!relay.durableWelcomeScheduling || result.durableWelcomeScheduling !== true)) {
...
export {
  __resetDenialCaptureDedupForTests,
  __setNotificationChannelsDepsForTests,
  config,
  handler as default
};
