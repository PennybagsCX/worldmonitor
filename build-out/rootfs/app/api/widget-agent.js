...
var RELAY_BASE = "https://proxy.worldmonitor.app";
var WIDGET_AGENT_KEY = process.env.WIDGET_AGENT_KEY ?? "";
var PRO_WIDGET_KEY = process.env.PRO_WIDGET_KEY ?? "";
var WORLDMONITOR_VALID_KEYS = (process.env.WORLDMONITOR_VALID_KEYS ?? "").split(",").map((v) => v.trim()).filter(Boolean);
var WIDGET_AGENT_BODY_TIMEOUT_MS = Number(process.env.WIDGET_AGENT_BODY_TIMEOUT_MS) || 5e3;
async function readRequestBody(req) {
  return Promise.race([
    req.text(),
    new Promise(
      (_, reject) => setTimeout(() => reject(new Error("widget-agent body read timeout")), WIDGET_AGENT_BODY_TIMEOUT_MS)
    )
  ]);
}
async function hasValidWorldMonitorKey(key) {
...
async function handler(req) {
  ...
  let isPro = false;
  const headerWorldMonitorKey = req.headers.get("X-WorldMonitor-Key") ?? req.headers.get("X-Api-Key") ?? "";
  const worldMonitorKey = headerWorldMonitorKey || getCookie(req, "wm-pro-key") || getCookie(req, "wm-widget-key");
  if (await hasValidWorldMonitorKey(worldMonitorKey)) {
    ...
    if (authHeader?.startsWith("Bearer ")) {
      ...
      if (!allowed) {
        ...
        const reason = entitlementChecked && entitlementTier === null ? "service_unavailable" : "not_entitled";
