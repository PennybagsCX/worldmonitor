...
async function verifyBriefToken(userId, issueDate, token, secret, prevSecret) {
...
var USER_ID_RE2 = /^[A-Za-z0-9_-]{1,128}$/;
var ISSUE_DATE_RE2 = /^\d{4}-\d{2}-\d{2}-\d{4}$/;
...
function assertShape2(userId, issueDate) {
  if (!USER_ID_RE2.test(userId)) {
    throw new BriefShareUrlError("invalid_user_id", "userId must match [A-Za-z0-9_-]{1,128}");
  }
  if (!ISSUE_DATE_RE2.test(issueDate)) {
...
async function deriveShareHash(userId, issueDate, secret) {
  assertShape2(userId, issueDate);
  if (!secret) {
    throw new BriefShareUrlError("missing_secret", "BRIEF_SHARE_SECRET is not configured");
  }
  const bytes = await hmacSha2562(secret, `${userId}:${issueDate}`);
  return base64url(bytes.slice(0, 9));
}
async function buildPublicBriefUrl({
  userId,
  issueDate,
  baseUrl,
  secret,
...
function encodePublicPointer(userId, issueDate) {
  assertShape2(userId, issueDate);
...
function renderErrorPage(heading, body) {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${heading} \xB7 WorldMonitor</title><style>${ERROR_PAGE_STYLES}</style></head><body><div><h1>${heading}</h1><p>${body}</p><p><a href="https://worldmonitor.app">Return to WorldMonitor</a></p></div></body></html>`;
}
var EXPIRED_PAGE = renderErrorPage(
  "This brief has expired.",
  "Briefs are kept for seven days after they are issued. Your next brief will be delivered on schedule."
);
var FORBIDDEN_PAGE = renderErrorPage(
...
var FOLLOWED_COUNTRIES_TIMEOUT_MS = 500;
async function fetchFollowedCountriesEdge(userId, ctx) {
  const convexSiteUrl = process.env.CONVEX_SITE_URL ?? (process.env.CONVEX_URL ?? "").replace(".convex.cloud", ".convex.site");
...
async function handler(req, ctx) {
  ...
  const prevSecret = process.env.BRIEF_URL_SIGNING_SECRET_PREV || void 0;
  if (!secret) {
  ...
  try {
    ...
    console.error("[api/brief] Upstash read failed:", err.message);
  ...
  if (shareSecret) {
    try {
      ...
      console.warn("[api/brief] share URL derive failed:", err.message);
  ...
  try {
    ...
    console.error("[api/brief] malformed envelope for brief:*:*:", err.message);
