...
var ACLED_CLIENT_ID = "acled";
var EXPIRY_MARGIN_MS = 5 * 60 * 1e3;
var REDIS_CACHE_KEY = "acled:oauth:token";
var REDIS_TTL_SECONDS = 23 * 60 * 60;
var memCached = null;
var refreshPromise = null;
async function requestAcledToken(body, action) {
...
async function exchangeCredentials(email, password) {
  ...
  return {
    ...
    expiresAt: Date.now() + (data.expires_in ?? 86400) * 1e3
  };
}
...
async function cacheToRedis(state) {
  try {
...
async function restoreFromRedis() {
  try {
    ...
    if (data && typeof data === "object" && "accessToken" in data && "refreshToken" in data && "expiresAt" in data) {
      return data;
    }
  } catch (err) {
...
// server/worldmonitor/conflict/v1/list-acled-events.ts
var REDIS_CACHE_KEY2 = "conflict:acled:v1";
var REDIS_CACHE_TTL = 900;
var ACLED_DEFAULT_WINDOW_MS = 30 * 24 * 60 * 60 * 1e3;
var fallbackAcledCache = /* @__PURE__ */ new Map();
function resolveAcledEventWindow(req, now = Date.now()) {
  return {
    startMs: req.start > 0 ? req.start : now - ACLED_DEFAULT_WINDOW_MS,
    endMs: req.end > 0 ? req.end : now
  };
}
async function fetchAcledConflicts(req, window) {
  try {
...
async function listAcledEvents(_ctx, req) {
  ...
  const cacheKey = `${REDIS_CACHE_KEY2}:${req.country || "all"}:${window.startMs}:${window.endMs}`;
  try {
    const result = await cachedFetchJson(
      cacheKey,
      REDIS_CACHE_TTL,
...
async function listUcdpEvents(ctx, req) {
  try {
    const raw = await getCachedJson(CACHE_KEY, true);
    if (!raw?.events?.length) return markNoStoreFallbackResponse(ctx.request, { events: [], pagination: void 0 });
    let events = raw.events;
    if (req.country) events = events.filter((e) => e.country === req.country);
    return { events, pagination: void 0 };
  } catch {
    return markNoStoreFallbackResponse(ctx.request, { events: [], pagination: void 0 });
  }
}
...
async function getHumanitarianSummary(_ctx, req) {
  ...
  try {
    ...
    const cached = await getCachedJson(`${REDIS_CACHE_KEY3}:${countryCode}`);
    return cached ?? { summary: void 0 };
  } catch {
...
// server/worldmonitor/conflict/v1/get-humanitarian-summary-batch.ts
var REDIS_CACHE_KEY4 = "conflict:humanitarian:v1";
...
async function getHumanitarianSummaryBatch(_ctx, req) {
  try {
    ...
    const results = {};
    const cacheKeys = limitedList.map((cc) => `${REDIS_CACHE_KEY4}:${cc}`);
