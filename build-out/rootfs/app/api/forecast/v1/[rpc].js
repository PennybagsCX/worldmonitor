...
function createForecastServiceRoutes(handler, options) {
  return [
    {
      method: "GET",
...
var REDIS_KEY = "forecast:scorecard:v1";
var MAX_STALE_MS = 2160 * 60 * 1e3;
function emptyScorecard(overrides = {}) {
  return {
    schemaVersion: 1,
    generatedAt: 0,
...
var getForecastScorecard = async (ctx) => {
  try {
...
async function getScorecardJson() {
...
function isScorecardSeedEnvelope(value) {
  if (value == null || typeof value !== "object" || Array.isArray(value)) return false;
  const seed = value._seed;
  return seed != null && typeof seed === "object" && "fetchedAt" in seed;
}
async function getRawString(key) {
  const url = process.env.UPSTASH_REDIS_REST_URL;
  const token = process.env.UPSTASH_REDIS_REST_TOKEN;
  if (!url || !token) throw new Error("Redis credentials not configured");
  const resp = await fetch(`${url}/get/${encodeURIComponent(key)}`, {
...
init_redis();
var REDIS_KEY2 = "forecast:predictions:v2";
var FORECAST_DOMAINS = new Set(openapi_filter_param_contracts_default.forecastDomains);
var getForecasts = async (ctx, req) => {
  try {
    const data = await getRawJson(REDIS_KEY2);
    if (!data?.predictions) {
...
var SIMULATION_PACKAGE_LATEST_KEY = "forecast:simulation-package:latest";
...
// server/worldmonitor/forecast/v1/get-simulation-package.ts
function isPackagePointer(v) {
  if (!v || typeof v !== "object") return false;
  const o = v;
  return typeof o["runId"] === "string" && typeof o["pkgKey"] === "string" && typeof o["schemaVersion"] === "string" && typeof o["theaterCount"] === "number" && typeof o["generatedAt"] === "number";
}
var NOT_FOUND = {
  found: false,
  runId: "",
...
var getSimulationPackage = async (ctx, req) => {
  try {
    ...
    const note = req.runId && req.runId !== pointer.runId ? "runId filter not yet active; returned package may differ from requested run" : "";
...
var SIMULATION_TASK_KEY_PREFIX = "forecast:simulation-task:v1";
var SIMULATION_TASK_QUEUE_KEY = "forecast:simulation-task-queue:v1";
var SIMULATION_TASK_TTL_SECONDS = 4 * 60 * 60;
var SIMULATION_OUTCOME_LATEST_KEY = "forecast:simulation-outcome:latest";
var SIMULATION_OUTCOME_BY_RUN_KEY_PREFIX = "forecast:simulation-outcome:by-run";
var SIMULATION_OUTCOME_BY_RUN_TTL_SECONDS = 24 * 60 * 60;
var SIMULATION_PACKAGE_LATEST_KEY2 = "forecast:simulation-package:latest";
var MAX_QUEUE_DEPTH = 100;
var VALID_RUN_ID_RE = /^\d{13,}-[a-z0-9-]{1,64}$/i;
var SIMULATION_TRIGGER_RATE_LIMIT = Object.freeze({ limit: 10, window: "60 s" });
async function pkgFingerprint(pkgKey) {
...
init_redis();
var TASK_QUEUE_TTL_SECONDS = 60 * 24 * 60 * 60;
var REDIS_READ_TIMEOUT_MS = 5e3;
async function redisGetThrowing(key) {
  const url = process.env.UPSTASH_REDIS_REST_URL;
  const token = process.env.UPSTASH_REDIS_REST_TOKEN;
  if (!url || !token) throw new Error("Redis credentials not configured");
  const resp = await fetch(`${url}/get/${encodeURIComponent(key)}`, {
...
async function enqueueSimulationTaskForServer(runId, pkgFingerprintValue) {
...
async function getSimulationPackagePointer() {
  const raw = await redisGetThrowing(SIMULATION_PACKAGE_LATEST_KEY2);
  if (!raw) return null;
  let parsed;
  try {
...
async function getSimulationOutcomeLatest() {
...
async function listProcessingRunIds(limit = 100) {
  const [entry] = await runRedisPipeline(
    [["ZRANGE", SIMULATION_TASK_QUEUE_KEY, "0", String(Math.max(0, limit - 1))]],
    true
  );
  return Array.isArray(entry?.result) ? entry.result.filter((v) => typeof v === "string") : [];
}

// server/worldmonitor/forecast/v1/get-simulation-outcome.ts
function isOutcomePointer(v) {
  if (!v || typeof v !== "object") return false;
  const o = v;
  return typeof o["runId"] === "string" && typeof o["outcomeKey"] === "string" && typeof o["schemaVersion"] === "string" && typeof o["theaterCount"] === "number" && typeof o["generatedAt"] === "number";
}
...
function outcomeToResponse(pointer, note) {
  const theaterSummariesJson = Array.isArray(pointer.uiTheaters) && pointer.uiTheaters.length > 0 ? JSON.stringify(pointer.uiTheaters) : "";
  return {
    found: true,
    runId: pointer.runId,
...
var getSimulationOutcome = async (ctx, req) => {
  if (req.runId) {
    let byRunRaw = null;
    try {
    ...
    if (isTombstone(byRunRaw)) {
      ...
      markNoCacheResponse(ctx.request);
      return { ...NOT_FOUND2, error: "redis_unavailable" };
    }
    try {
  ...
  try {
    ...
    const note = req.runId && req.runId !== pointer.runId ? "requested runId not found (may have expired beyond 24h retention); returned latest available outcome instead" : "";
...
function createPremiumRpcBillingDenialError(identity, ApiErrorConstructor) {
...
async function requirePremiumRpcAccess(request, ApiErrorConstructor, fallbackMessage) {
...
async function triggerSimulation(ctx, req) {
  ...
  const authKind = apiKeyHeader ? apiKeyHeader.startsWith("wm_") ? "user_api_key" : "enterprise_api_key" : authHeader ? "clerk_jwt" : "unknown";
