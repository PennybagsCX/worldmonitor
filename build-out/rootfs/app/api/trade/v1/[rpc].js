...
async function validateUserApiKey(key) {
  ...
  try {
    ...
    const unavailable3 = toUnavailableError(err);
    console.warn("[user-api-key] validateUserApiKey unavailable:", unavailable3.message);
    throw unavailable3;
  }
}
...
async function getEntitlements(userId) {
  const unavailable3 = _unavailableUntil.get(userId);
  if (unavailable3 !== void 0) {
    if (unavailable3.expiresAt > Date.now()) {
      return unavailableEntitlements(unavailable3.retryAfterSeconds);
    }
...
init_redis();
var SEED_CACHE_KEY = "trade:restrictions:v1:tariff-overview:50";
async function getTradeRestrictions(_ctx, req) {
  try {
    ...
    if (!result?.restrictions?.length) {
      return { restrictions: [], fetchedAt: (/* @__PURE__ */ new Date()).toISOString(), upstreamUnavailable: true };
    }
...
// server/worldmonitor/trade/v1/get-tariff-trends.ts
var SEED_KEY_PREFIX = "trade:tariffs:v2";
var COVERAGE_KEY = `${SEED_KEY_PREFIX}:index`;
var DEFAULT_REPORTING_COUNTRY = "840";
var DEFAULT_YEARS = 10;
var MAX_YEARS = 30;
var COVERED_PRODUCT_SECTOR = "all";
var REASON = {
...
var CODE_PATTERN = /^[0-9]{3}$/;
var SECTOR_PATTERN = /^[a-zA-Z0-9_-]{0,16}$/;
var REPORTER_ID_PATTERN = /^[0-9]{3}$/;
var LEGACY_SEEDED_YEARS = 10;
var LEGACY_KEY_PREFIX = "trade:tariffs:v1";
function tariffTrendSeedKey(reporter) {
  return `${SEED_KEY_PREFIX}:${reporter}`;
}
function legacyTariffTrendSeedKey(reporter) {
  return `${LEGACY_KEY_PREFIX}:${reporter}:${COVERED_PRODUCT_SECTOR}:${LEGACY_SEEDED_YEARS}`;
}
function tariffTrendCoverageId(reporter) {
  return reporter;
}
function normalizeTariffTrendRequest(req) {
  ...
  return {
    ...
    partner: rawPartner,
    productSector: sectorLower === "" || sectorLower === COVERED_PRODUCT_SECTOR ? COVERED_PRODUCT_SECTOR : sectorLower,
    years: rawYears === 0 ? DEFAULT_YEARS : rawYears
  };
}
function sliceTariffsToWindow(datapoints, years) {
  ...
  return datapoints.filter((point) => Number.isFinite(point?.year) && point.year >= startYear && point.year <= endYear).sort((a, b) => a.year - b.year);
}
function unavailable(reason, upstreamUnavailable) {
  return {
    datapoints: [],
...
function logReadFailure(key, error) {
  if (error instanceof Error && (error.name === "TimeoutError" || error.name === "AbortError")) {
    console.error(`[REDIS-TIMEOUT] readCachedJson key=${key}`);
  }
  const message2 = error instanceof Error ? error.message : String(error);
  console.warn(`[tariff-trends] ${key} read failed: ${message2}`);
}
async function classifyMiss(reporter) {
  ...
  const manifest = read.status === "hit" ? read.value : null;
  const reporters = manifest?.reporters;
  if (!Array.isArray(reporters) || !reporters.every((id) => typeof id === "string" && REPORTER_ID_PATTERN.test(id))) {
    return unavailable(REASON.coverageUnknown, true);
  }
  return reporters.includes(tariffTrendCoverageId(reporter)) ? unavailable(REASON.seedMissing, true) : unavailable(REASON.notCovered, false);
}
async function readLegacySeed(reporter, years) {
  ...
  const seeded = read.status === "hit" ? read.value : null;
  const datapoints = Array.isArray(seeded?.datapoints) ? sliceTariffsToWindow(seeded.datapoints, years) : [];
  if (datapoints.length === 0) return null;
  return served(datapoints, seeded?.fetchedAt, seeded?.effectiveTariffRate);
}
function served(datapoints, fetchedAt, effectiveTariffRate) {
...
async function getTariffTrends(ctx, req) {
...
init_redis();
var SEED_KEY_PREFIX2 = "trade:flows:v2";
var COVERAGE_KEY2 = `${SEED_KEY_PREFIX2}:index`;
var DEFAULT_REPORTING_COUNTRY2 = "840";
var WORLD_PARTNER_CODE = "000";
var DEFAULT_YEARS2 = 10;
var MAX_YEARS2 = 30;
var REASON2 = {
...
var CODE_PATTERN2 = /^[0-9]{3}$/;
var PAIR_ID_PATTERN = /^[0-9]{3}:[0-9]{3}$/;
function tradeFlowSeedKey(reporter, partner) {
  return `${SEED_KEY_PREFIX2}:${reporter}:${partner}`;
}
function tradeFlowCoverageId(reporter, partner) {
  return `${reporter}:${partner}`;
}
function normalizeTradeFlowRequest(req) {
  const rawReporter = (req.reportingCountry ?? "").trim();
  const rawPartner = (req.partnerCountry ?? "").trim();
  if (rawReporter !== "" && !CODE_PATTERN2.test(rawReporter)) return null;
  if (rawPartner !== "" && !CODE_PATTERN2.test(rawPartner)) return null;
  const rawYears = req.years;
  if (typeof rawYears !== "number" || !Number.isInteger(rawYears)) return null;
  if (rawYears < 0 || rawYears > MAX_YEARS2) return null;
  return {
    reporter: rawReporter === "" ? DEFAULT_REPORTING_COUNTRY2 : rawReporter,
    partner: rawPartner === "" ? WORLD_PARTNER_CODE : rawPartner,
    years: rawYears === 0 ? DEFAULT_YEARS2 : rawYears
  };
}
function sliceFlowsToWindow(flows, years) {
  ...
  if (!Number.isFinite(endYear)) return [];
  const startYear = endYear - years;
  return flows.filter((flow) => Number.isFinite(flow?.year) && flow.year >= startYear && flow.year <= endYear).sort((a, b) => a.year - b.year);
}
function unavailable2(reason, upstreamUnavailable) {
  return {
    flows: [],
    // Nothing was fetched, so there is no fetch time to report. Stamping "now"
    // here — as this handler used to — dates an empty answer as if it were a
...
function logReadFailure2(key, error) {
  if (error instanceof Error && (error.name === "TimeoutError" || error.name === "AbortError")) {
    console.error(`[REDIS-TIMEOUT] readCachedJson key=${key}`);
  }
  const message2 = error instanceof Error ? error.message : String(error);
  console.warn(`[trade-flows] ${key} read failed: ${message2}`);
}
async function classifyMiss2(reporter, partner) {
  const read = await readCachedJson(COVERAGE_KEY2, true);
  if (read.status === "error") {
    logReadFailure2(COVERAGE_KEY2, read.error);
    return unavailable2(REASON2.cacheUnavailable, true);
  }
  const manifest = read.status === "hit" ? read.value : null;
  const pairs = manifest?.pairs;
  if (!Array.isArray(pairs) || !pairs.every((id) => typeof id === "string" && PAIR_ID_PATTERN.test(id))) {
    return unavailable2(REASON2.coverageUnknown, true);
  }
  return pairs.includes(tradeFlowCoverageId(reporter, partner)) ? unavailable2(REASON2.seedMissing, true) : unavailable2(REASON2.notCovered, false);
}
async function getTradeFlows(_ctx, req) {
  const normalized = normalizeTradeFlowRequest(req);
  if (!normalized) return unavailable2(REASON2.invalidRequest, false);
  const { reporter, partner, years } = normalized;
  const seedKey = tradeFlowSeedKey(reporter, partner);
  const read = await readCachedJson(seedKey, true);
  if (read.status === "error") {
    logReadFailure2(seedKey, read.error);
    return unavailable2(REASON2.cacheUnavailable, true);
  }
  const seeded = read.status === "hit" ? read.value : null;
  const flows = Array.isArray(seeded?.flows) ? sliceFlowsToWindow(seeded.flows, years) : [];
  if (flows.length === 0) return classifyMiss2(reporter, partner);
  return served2(flows, seeded?.fetchedAt);
}
function served2(flows, fetchedAt) {
  const first = flows[0];
  ...
  return {
    ...
    fetchedAt: typeof fetchedAt === "string" ? fetchedAt : "",
    upstreamUnavailable: false,
    unavailableReason: REASON2.served,
    coverageStartYear: first?.year ?? 0,
    coverageEndYear: last?.year ?? 0
  };
}
...
init_redis();
var SEED_CACHE_KEY2 = "trade:barriers:v1:tariff-gap:50";
async function getTradeBarriers(_ctx, req) {
  try {
    const result = await getCachedJson(SEED_CACHE_KEY2, true);
    if (!result?.barriers?.length) {
      return { barriers: [], fetchedAt: (/* @__PURE__ */ new Date()).toISOString(), upstreamUnavailable: true };
    }
    const limit = Math.max(1, Math.min(req.limit > 0 ? req.limit : 50, 100));
    return {
...
// server/worldmonitor/trade/v1/get-customs-revenue.ts
init_redis();
var CUSTOMS_KEY = "trade:customs-revenue:v1";
async function getCustomsRevenue(_ctx, _req) {
  try {
    const data = await getCachedJson(CUSTOMS_KEY, true);
    if (data?.months?.length) return data;
    return { months: [], fetchedAt: (/* @__PURE__ */ new Date()).toISOString(), upstreamUnavailable: true };
  } catch (err) {
...
// scripts/shared/comtrade-strategic-products.json
var comtrade_strategic_products_default = {
  schemaVersion: 1,
  classification: {
    code: "H6",
    name: "HS2022",
  ...
  products: [
    { id: "semiconductors", label: "Semiconductors", tradeFlowCode: "8542", tradeFlowCoverageStage: 1, bilateralHs4Code: "8542" },
    { id: "batteries", label: "Lithium-ion batteries", tradeFlowCode: "850760", tradeFlowCoverageStage: 2 },
    { id: "electric_vehicles", label: "Electric vehicles", bilateralLabel: "Passenger vehicles (including EVs)", tradeFlowCode: "870380", tradeFlowCoverageStage: 2, bilateralHs4Code: "8703" },
    { id: "rare_earth_inputs", label: "Rare-earth metals", tradeFlowCode: "280530", tradeFlowCoverageStage: 2 },
    { id: "solar_cells", label: "Photovoltaic cells (unassembled)", tradeFlowCode: "854142", tradeFlowCoverageStage: 2 },
    { id: "solar_products", label: "Photovoltaic modules and panels", tradeFlowCode: "854143", tradeFlowCoverageStage: 2 },
    { id: "crude_oil", label: "Crude petroleum", tradeFlowCode: "2709", tradeFlowCoverageStage: 1, bilateralHs4Code: "2709" },
    { id: "petroleum_gas_legacy", label: "Petroleum gas (legacy HS4)", tradeFlowCode: "2711", tradeFlowCoverageStage: 1 },
    { id: "lng", label: "Liquefied natural gas", bilateralLabel: "LNG and petroleum gas", tradeFlowCode: "271111", tradeFlowCoverageStage: 2, bilateralHs4Code: "2711" },
    { id: "coal", label: "Coal", tradeFlowCode: "2701", tradeFlowCoverageStage: 2 },
    { id: "iron_ore", label: "Iron ore and concentrates", tradeFlowCode: "2601", tradeFlowCoverageStage: 2 },
    { id: "copper", label: "Copper ore and concentrates", tradeFlowCode: "2603", tradeFlowCoverageStage: 2 },
    { id: "gold", label: "Gold", tradeFlowCode: "7108", tradeFlowCoverageStage: 1, bilateralHs4Code: "7108" },
    { id: "arms", label: "Arms and military equipment", tradeFlowCode: "9301", tradeFlowCoverageStage: 1 },
    { id: "telecom_equipment", label: "Smartphones and telecom equipment", bilateralHs4Code: "8517" },
    { id: "pharmaceuticals", label: "Pharmaceuticals", bilateralHs4Code: "3004" },
    { id: "refined_petroleum", label: "Refined petroleum", bilateralHs4Code: "2710" },
    { id: "computers", label: "Computers", bilateralHs4Code: "8471" },
    { id: "turbines", label: "Turbojets and turbines", bilateralHs4Code: "8411" },
    { id: "aluminium", label: "Aluminium", bilateralHs4Code: "7601" },
    { id: "ferroalloys", label: "Ferroalloys (steel)", bilateralHs4Code: "7202" },
    { id: "polyethylene", label: "Plastics (polyethylene)", bilateralHs4Code: "3901" },
    { id: "hydrocarbon_chemicals", label: "Chemicals (hydrocarbons)", bilateralHs4Code: "2902" },
    { id: "wheat", label: "Wheat", bilateralHs4Code: "1001" },
    { id: "soybeans", label: "Soybeans", bilateralHs4Code: "1201" },
    { id: "garments", label: "Garments", bilateralHs4Code: "6204" },
    { id: "pork", label: "Pork", bilateralHs4Code: "0203" },
    { id: "trucks", label: "Trucks", bilateralHs4Code: "8704" },
    { id: "vehicle_parts", label: "Vehicle parts", bilateralHs4Code: "8708" }
  ]
};

// server/worldmonitor/trade/v1/list-comtrade-flows.ts
init_redis();
var KEY_PREFIX = "comtrade:flows";
var REPORTERS = ["842", "156", "643", "364", "699", "490"];
var CMD_CODES = comtrade_strategic_products_default.products.map((product) => product.tradeFlowCode).filter((code) => typeof code === "string" && code.length > 0);
var CMD_CODE_RE = new RegExp(openapi_filter_param_contracts_default.tradeComtradeCmdCodePattern);
function isValidCode(c) {
  return /^\d{1,10}$/.test(c);
}
async function listComtradeFlows(ctx, req) {
  const isPro = await isCallerPremium(ctx.request);
  ...
  try {
    ...
    for (const result of batch.values()) {
      if (!result) continue;
      dataFound = true;
      const records = Array.isArray(result) ? result : result.flows ?? [];
      if (!fetchedAt && result.fetchedAt) {
