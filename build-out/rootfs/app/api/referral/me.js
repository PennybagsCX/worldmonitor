...
function hexHmac(secret, message2) {
  return (async () => {
    ...
    const sig = await crypto.subtle.sign("HMAC", key, enc.encode(message2));
...
async function getReferralCodeForUser(userId, secret) {
...
var PUBLIC_BASE = process.env.WORLDMONITOR_PUBLIC_BASE_URL ?? "https://worldmonitor.app";
async function registerReferralCodeInConvex(userId, code) {
  const convexSite = process.env.CONVEX_SITE_URL ?? (process.env.CONVEX_URL ?? "").replace(".convex.cloud", ".convex.site");
...
async function handler(req, ctx) {
  ...
  try {
    ...
    console.error("[api/referral/me] code generation failed:", err.message);
    captureSilentError(err, { tags: { route: "api/referral/me", step: "code-generation" }, ctx });
    return jsonResponse({ error: "service_unavailable" }, 503, cors);
  }
  ctx.waitUntil(
    registerReferralCodeInConvex(session.userId, code).catch((err) => {
      console.warn(
