...
自 v1.6.0 起，每个工具的 `tools/list` 条目都声明了规范定义的 [MCP 2025-06-18 `Tool.outputSchema`](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#tool-result-schema)。该 schema 描述 `result.structuredContent`（即 `result.content[0].text` 所序列化的同一文档）；其第一个 `anyOf` 分支是工具的文档化负载形状 — 让客户端无需发起真正的 `tools/call` 即可编写投影、验证响应或生成类型。
