...
2. **One abort reason reaches every hook.** The hook shape reproduced here was `const p = orig(...); p.then(onOk); return p;`. The forked `p.then(onOk)` promise rejects with the same object and has no handler. The original `p` travels up our wrappers to Sentry's outermost wrapper, which backfills the stack during the same microtask turn. `unhandledrejection` fires only after that turn ends, so Sentry's global handler reads a reason that already has insights-loader frames. Production evidence, per this session's pull: one affected user's console breadcrumbs showed `chrome-extension://hoklmmgfnpapgjgcpechhaamimifchmp/frame_ant/frame_ant.js` wrapping `window.fetch`, then `chrome-extension://dbjbempljhcmhlfpfacalomonjpalpko/scripts/inspector.js` wrapping it, then our `main-*.js`, with the log "Fetch request failed:". Adding that one hook in the browser reproduction produced the exact production event.
...
A native `AbortSignal.timeout` reason already has `stack === "TimeoutError: signal timed out"` (checked in Chromium in this investigation). That value isn't `undefined`, so the check at `fetch.js:100` skips it, the SDK parses zero frames, and the suppression drops it. That's why only the hand-built reason ever surfaced. Once the loader's reason has the same shape, a leaked rejection is filtered the same way. A real first-party failure still gets through because it's reported with a `kind` tag, which exempts it at `src/bootstrap/sentry-init.ts:1003`.

Trade-off: if our own code ever left this particular reason unhandled, Sentry would drop it too. That's acceptable because first-party failures that must be seen go through `kind`-tagged captures like `panel_call_rejected`, not bare unhandled rejections.
...
**Rule for JS-built abort reasons.** Anything we pass to `AbortController.abort()` that a hooked `fetch` might reject with should look like the native reason: a `stack` that is only the header line, set with `Object.defineProperty`. A `stack` of `undefined` invites Sentry to backfill it. A real stack with frames makes the event first-party. One known place still builds a bare reason: the fallback at `src/services/timeout-signal.ts:40-44`. It runs only where `AbortSignal.timeout` is missing (`src/services/timeout-signal.ts:27-29`). If that path starts showing up with call-site frames, give it the same stamp.
